...
switch ( $action ){
	case "add":
		...
		if( $total_max_cl >= MAX_COVER_LETTER ){
			...
			redirect_to( BASE_URL."covering_letter/?portid=".$intPortId);
			die;
		}
	...
	default:
		if( $total_max_cl <=0 && $action != "add" ) redirect_to( BASE_URL."covering_letter/add/?portid=".$intPortId);
