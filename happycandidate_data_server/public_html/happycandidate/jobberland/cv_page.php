<?php 
	...
	$_SESSION['direct_to'] = BASE_URL."curriculum_vitae/?portid=".$intPortId; 	
...
switch ( $action ){
	...
	case "copy":
		...
		if ( copy ( $old , $new ) ){
			if ( $copy_cv->save() )
			{
				...
				redirect_to( BASE_URL."curriculum_vitae/rename/".$db->insert_id()."/?portid=".$intPortId);
				exit;
			}else{
	...
	case "default":
		...
		if( $cv_setting->make_defult() ){ redirect_to( BASE_URL."curriculum_vitae/?portid=".$intPortId); }
		exit;
	break;
	...
	if( $total_max_cv <=0 && $action != "add" ) redirect_to( BASE_URL."curriculum_vitae/add/?portid=".$intPortId );
