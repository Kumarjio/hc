<?php $_SESSION['direct_to_emp'] = "addjob/"; 	
	...
	if ( isset($_POST['bt_add']) ){
		...
		$_SESSION['add_job']['compname'] = $job->company_name 	= stripHTMLTags($_POST['txt_cname']);
		...
		if ( is_array($_POST['txt_job_status']) && !empty($_POST['txt_job_status']) ) {
			...
			foreach ( $_POST['txt_job_status'] as $key => $value ):
				...
				//$arrJobdetail['txt_job_statu'] = $value;
			endforeach;
		}
		...
		if ( is_array($_POST['txt_education']) && !empty($_POST['txt_education']) ) {
			...
			foreach ( $_POST['txt_education'] as $key => $value ):
				...
				$arrJobdetail['txt_education'] = (int)$value;
			endforeach;
		}
		...
		if ( is_array($_POST['txt_experience']) && !empty($_POST['txt_experience']) ) {
			...
			foreach ( $_POST['txt_experience'] as $key => $value ):
				...
				$arrJobdetail['txt_experience'] = (int)$value;
			endforeach;
		}
...
###############localtion
		...
		$arrJobdetail['txt_pcode'] = $_SESSION['loc']['pcode']	= $job->post_code 			= stripHTMLTags($_POST['txt_pcode']);
		$arrJobdetail['txt_location'] = $_SESSION['loc']['citycode']	= $job->city 			= stripHTMLTags($_POST['txtcity']);
		...
		if ( is_array($_POST['txt_category']) && !empty($_POST['txt_category']) ) {
			...
			foreach ( $_POST['txt_category'] as $key => $value ):
				...
				$arrJobdetail['txt_category'][] = $value;
			endforeach;
		}
		$job->category 	= (int)sizeof($_POST['txt_category']);
		$smarty->assign( 'category_selected', $category_selected );
		$job->fk_hc_employer_id=$_SESSION['BackendUser']['hcuid'];
		$job->fk_hc_portal_id=$_SESSION['BackendUser']['portal_id'];
		...
		if( $job->save() )
		{
			...
			if($status_added && $cat_added && $type_added )
			{
				...
				// add code to get the list of employee matching this job and send system and mail notification about the job
				//$arrCandidates = Job::find_matched_cvs($arrJobdetail);
				/*if(is_array($arrCandidates) && (count($arrCandidates)>0))
				{
					$arrNewJobDetail = array();
					$arrNewJobDetail['newjobname'] = $_POST['txt_job_title'];
					$arrNewJobDetail['newjobid'] = $job_id;
					$arrNewJobDetail['newjobcreatedby'] = $_SESSION['BackendUser']['hcuid'];
					$arrNewJobDetail['createdon'] = date("Y-m-d H:i:s");
					
					$arrNotificationResult =  $job->new_job_notification($arrCandidates,$arrNewJobDetail);
				}*/
			}else{
...
//end of location 
	...
	$smarty->assign('left_side', $smarty->fetch('employer/left_job.tpl') );
