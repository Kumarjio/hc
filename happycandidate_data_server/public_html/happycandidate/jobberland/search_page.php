...
//get total number of records 
	$select = " SELECT job.id as job_id, job.var_name as job_var_name, job.job_title, job.job_description, job.city, job.fk_employer_id as employer_id, employer.company_name,employer.var_name as company_var_name, jobtype.type_name, job.job_salary, job.salaryfreq, job.created_at  ";
		...
		$where = " WHERE DATE_ADD( job.created_at, INTERVAL ".JOBLASTFOR." DAY ) > NOW() 
					...
					AND job.job_status = 'approved' 
					AND job.country='".$country_code."' 
					AND fk_hc_portal_id = '".$_GET['portid']."'";
		...
		if ( $q != "" )
		{
			$strSearchType = $_GET['search_type'];
			if($strSearchType == "1")
			{
				$q = str_replace("\"","",$q);
				$q = str_replace("+","",$q);
...
if( !empty($_GET) ){
	...
	//echo "---".$query;
	$arrReferenceName = array();
	if(isset($_GET['q']))
	{
		//$q = $_GET['q'];
		$arrRefereceName[] = $_GET['q'];
	}
	
	if(isset($_GET['location']))
	{
		$arrRefereceName[] = $_GET['location'];
	}
	
	if(isset($_GET['txt_country']))
	{
		$arrRefereceName[] = $_GET['txt_country'];
	}
	
	if(isset($_GET['category']))
	{
		$arrCat = $_GET['category'];
		$arrRefereceName[] = $arrCat[0];
	}
	
	if(isset($_GET['experience']))
	{
		$arrRefereceName[] = $_GET['experience'];
	}
	
	if(isset($_GET['job_type']))
	{
		$arrRefereceName[] = $_GET['job_type'];
	}
	
	$q = $strRefeName = implode("-",$arrRefereceName);
	
	
	//$query = substr_replace($query, '', 0, 3);		
	$smarty->assign( 'query', $query );
	$smarty->assign( 'save_reference_name', $q );
	//$smarty->assign( 'save_reference', urlencode($query) );
	$smarty->assign( 'save_reference',base64_encode($query));
	$smarty->assign( 'port_id',$_GET['portid']);
}
