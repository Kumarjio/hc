...
###############################################################################
		...
		$total_count = Employer::count_all_owner();
		...
		$sql=" SELECT * FROM ".TBL_EMPLOYER.", user WHERE jobberland_employer.hc_employer_id = user.id AND user.portal_id != '0'";
		$sql.=" LIMIT {$per_page} ";
		$sql.=" OFFSET {$offset} ";
		...
		if($lists && is_array($lists)){
			...
			// retrieving portal name
			  ...
			  {
				$arrEmployerPortalDetail = Employer::find_portal_by_employer_id($list->hc_employer_id);
							
				if(is_array($arrEmployerPortalDetail) && (count($arrEmployerPortalDetail)>0))
				{
					$manage_lists[$i]['portalid'] = $arrEmployerPortalDetail['career_portal_id'];
					
					if($arrEmployerPortalDetail['career_portal_name'])
					{
						$manage_lists[$i]['portalname'] = $arrEmployerPortalDetail['career_portal_name'];
					}
					else
					{
...
$smarty->assign('pagetitle', 'Owners');
$smarty->assign( 'message', $message );	
$smarty->assign('rendered_page', $smarty->fetch('admin/manage_owner.tpl') );
