...
###################### DELETE ####################################		
	...
	if(isset($_POST['delete_all']) && $_POST['employer_id'] != "" && sizeof($_POST['employer_id']) != 0 ){
		...
		if($success){
			$session->message("<div class='success'>Owner(s) has been deleted </div>");
...
###################### deactivate ####################################		
	if( isset($_GET['id']) && isset($_GET['action']) && $_GET['action'] == "deactivate" ){
		...
		if($employer->deactive_user())
		{
			$empUser = Employer::find_by_id($employer->id);
			$employer->hc_employer_id = $empUser->hc_employer_id;
			$employer->deactive_hc_user();
		}
	}
	if(isset($_POST['deactivate_all']) && $_POST['employer_id'] != "" && sizeof($_POST['employer_id']) != 0 ){
		...
		if($success){
			$session->message("<div class='success'>Owner(s) has been Inactivated </div>");
			redirect_to( $_SERVER['PHP_SELF'] );
			die;
		}
	}
...
###############################################################################
		...
		if($lists && is_array($lists)){
			...
			foreach( $lists as $list ):
			  ...
			  $manage_lists[$i]['login_as_url']		= HCBASEURL."loginas/login/0/".$list->hc_employer_id."/";
			  if($list->hc_employer_id)
			  {
				$arrPortalName = Employer::find_portal_by_employer_id($list->hc_employer_id);
				if(is_array($arrPortalName) && (count($arrPortalName)>0))
				{
					$manage_lists[$i]['employer_portal_name'] = $arrPortalName['career_portal_name'];
					$manage_lists[$i]['employer_portal_url'] = PORTALBASEURL.strtolower($arrPortalName['career_portal_name']);
				}
				else
				{
					$manage_lists[$i]['employer_portal_name'] = "";
				}
			  }
			  else
			  {
...
$html_title = SITE_NAME . " Add New Owner ";
