...
###################### deactivate ####################################		
	if( isset($_GET['id']) && isset($_GET['action']) && $_GET['action'] == "deactivate" ){
		$job->id = $_GET['id'];
		$job->deactive_job();
		$session->message("<div class='success'>Job(s) has been activated </div>");
		redirect_to( $_SERVER['PHP_SELF'] );
		die;
	}
	...
	if(isset($_POST['deactivate_all']) && $_POST['job_id'] != "" && sizeof($_POST['job_id']) != 0 ){
		...
		if($success){
			$session->message("<div class='success'>Job(s) has been Inactivated </div>");
			redirect_to( $_SERVER['PHP_SELF'] );
			die;
		}
	}
		...
		if($lists && is_array($lists)){
			...
			foreach( $lists as $list ):
			 ...
			 if( !empty($list->fk_employer_id) && $list->fk_employer_id != 0 ){
			  ...
			  if($list->fk_hc_portal_id)
			  {
				$arrPortalDetail = Employer::find_portal_by_id($list->fk_hc_portal_id);
				if(is_array($arrPortalDetail) && (count($arrPortalDetail)>0))
				{
					$manage_lists[$i]['portalid'] = $list->fk_hc_portal_id;
			  ...
			  $manage_lists[$i]['employer_name'] =  empty($employer) ? 'Owner not found' : $employer->full_name();
...
$smarty->assign('leftmenu', $smarty->fetch('admin/left_menu_job_listing.tpl'));
//$smarty->assign('leftmenu', $smarty->fetch('admin/left_menu_admin.tpl'));
$smarty->assign( 'pagetitle', "Job Listings" );	
