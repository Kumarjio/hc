<?php require_once( "../initialise_files.php" );  
	...
	if( isset($_POST['account_btn']) ){
		...
		$_SESSION['account']['hcuid'] 	= $employee->hc_uid 			= $hcuid;
		...
		$_SESSION['loc']['citycode']	= $employee->city 			= $_POST['localityval'];
		...
		$_SESSION['account']['pc'] 		= $employee->post_code 			= strtoupper($_POST['txt_pcode']);
		$_SESSION['account']['pn'] 		= $employee->phone_number 		= $_POST['txt_phone_number'];
		$_SESSION['account']['email'] = $employee->email_address 		= $_POST['txt_email_address'];
		$_SESSION['account']['pass'] = $employee->passwd 				= $_POST['txt_password'];
		...
		if($_POST['txt_password'])
		{
			...
			{
				$intSendPassword = $_POST['send_password'];
			}
		}
		/*print("<pre>");
		print_r($_POST);
		...
		if( $employee->save() ){
			if($intSendPassword)
			{
				// send changed password notification here
				
				$arrUserDetail = array();
				$arrUserDetail['name'] = $employee->fname." ".$employee->sname;
				$arrUserDetail['email'] = $employee->email_address;
				$arrUserDetail['newpassword'] = $_POST['txt_password'];
				
				
				$arrToMail = array();
				$arrToMail['email'] = $employee->email_address;
				$arrToMail['name'] = $employee->fname." ".$employee->sname;
				
				$arrFromMail = array();
				$arrFromMail['email'] = "admin@hc.com";
				$arrFromMail['name'] = "Admin HC";
				$strSubject = "Change Password Notification";
				$strMessageBody = fnChangePasswordNotificationMessage($arrUserDetail);
				
				$strChangePasswordNotification = send_mail($strMessageBody,$strSubject,$arrToMail,$arrFromMail);
			}
			destroy_my_session();
