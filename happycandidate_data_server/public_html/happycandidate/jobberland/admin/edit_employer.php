<?php  require_once( "../initialise_files.php" );  
	...
	/*print("<pre>");
	print_r($user);*/
	$intEmployerHcId = $user->hc_employer_id;
	...
	if( isset($_POST['bt_update']) ){
		...
		$employer->fname	 	 = $f_name 			= $_POST['txt_fname'];
		$employer->sname	 	 = $s_name 			= $_POST['txt_sname'];
		...
		$employer->passwd		= $emp_pass 		= $_POST['txt_password'];
		$employer->hc_employer_id	= $emp_hc_id 		= $intEmployerHcId;
		if($emp_pass)
		{
		...
		$_SESSION['loc']['citycode']	= $employer->city 			= $_POST['localityval'];
		...
		/*if( $_FILES['txt_logo']['error'] == 0 ) {
			...
			if($employer->save()) {
				...
				if($intSendPassword)
				{
					...
					$arrUserDetail = array();
					$arrUserDetail['name'] = $f_name." ".$s_name;
					$arrUserDetail['email'] = $email;
					$arrUserDetail['newpassword'] = $emp_pass;
					
					
					$arrToMail = array();
					$arrToMail['email'] = $email;
					$arrToMail['name'] = $f_name." ".$s_name;
				...
				redirect_to( 'manage_employer_active.php');
			}else {
				// Failure
