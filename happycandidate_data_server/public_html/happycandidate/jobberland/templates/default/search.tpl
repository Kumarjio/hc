<a href="{$BASE_URL}save_search/?action=save&name={$save_reference_name}&reference={$save_reference}&portid={$port_id}&set_default_search=1">Make Default Search</a>
&nbsp;<a href="{$BASE_URL}save_search/?action=save&name={$save_reference_name}&reference={$save_reference}&portid={$port_id}">Save Search</a>
...
{foreach from=$list_jobs key=k item=i}
  ...
  <tr>
    ...
    <td>
		{*<a href="{$BASE_URL}job/{$i.job_var_name}/">{$i.job_title}</a>*}
		<a href="javascript:void(0);">{$i.job_title}</a>
        <br /><br />
        {$i.job_description}
        ...
        <br /><br />
            ...
            <a href="javascript:void(0);">{$i.company_name}</a>
        <br />
    </td>
    <td>{$i.type_name}</td>
    <td>{$i.salary}</td>
    <td>
		{*<a href="{$BASE_URL}location/{$i.city_url}">{$i.location}</a>*}
		<a href="javascript:void(0);">{$i.location}</a>
         <br /><br />
          <a href="{$BASE_URL}save_job/?id={$i.id}&action=save_job&portid={$port_id}">{lang mkey='link' skey='save_job'}</a>
    </td>
  </tr>
