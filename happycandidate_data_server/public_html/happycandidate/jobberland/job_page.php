...
$job2type 	= new Job2Type();
	...
	$jobs = $job->find_by_id( $var_name );
	$job_id = (int)$jobs->id;
	
$jobapplied = new JobHistory();
if($user_id)
{
	$arrIsApplied = $jobapplied->check_user_already_apply($job_id,$user_id);
}
if($jobs && !empty($jobs) )
{
	...
	$smarty->assign('job_requirenment', 	$jobs->job_requirenment );
	...
	$compname = !empty($jobs->company_name) ? $jobs->company_name : format_lang('not_provided');	
	$smarty->assign('company_name', 	$compname );
	...
	$smarty->assign('user_applied',count($arrIsApplied));
	$smarty->assign('user_id',$user_id);
	$smarty->assign('intCandidateId', $_SESSION['PortalUser_'.$intLoggPortalId]['hcuid']);
	$smarty->assign('intPortId', $intPortId);
	$smarty->assign('intJobId', $job_id);
	$smarty->assign('interview_popup', $smarty->fetch('set_interview_reminder.tpl'));
