...
class CVExperiencePositionsSetting extends DatabaseObject{
	protected static $table_name = TBL_CVEXPERIENCEPOSITIONS;
	protected static $db_fields = array('experience_positions_details_id', 'exp_id', 'position_title','date_of_promotion','duration_year','duration_month','from_date','to_date','created_by');
	
	public $experience_positions_details_id;
	public $exp_id;
	public $position_title;
	public $date_of_promotion;
	...
	public function cv_review_by_employee(){
		global $database, $db;
		$sql=" SELECT * FROM ".self::$table_name;
		$sql.=" WHERE fk_employee_id=".(int)$this->fk_employee_id ." AND id=".$this->cv_experience_id;
		//echo $sql;
		$result = self::find_by_sql( $sql );
		return !empty($result) ? array_shift($result) : false;
	}
	...
	public function cv_review_by_employer(){
		global $database, $db;
		$sql=" SELECT * FROM ".self::$table_name;
		$sql.=" WHERE cv_status='Public' AND id=".$this->cv_experience_id;
	...
	public function update_no_views(){
		...
		$sql .= " SET no_views = no_views + 1 WHERE id=".$this->cv_experience_id;
		$database->query($sql);
	...
	public function make_defult(){
		...
		$sql.=" WHERE fk_employee_id='".$this->fk_employee_id."' AND id=".$this->cv_experience_id;
...
################################
	...
	public function delete_emp() {
	  ...
	  $sql .= " WHERE id=". $database->escape_value($this->cv_experience_id);
	  $sql .= " AND fk_employee_id='".$this->fk_employee_id."' ";
	  $sql .= " LIMIT 1";
	  $database->query($sql);
	  return ($database->affected_rows() == 1) ? true : false;
...
########################### SAVEING CV #####################################	
	...
	public function save() {
		...
		if( isset($this->experience_positions_details_id) ){
			
			if( !is_numeric($this->experience_positions_details_id) ){
				...
				if( sizeof($this->errors) == 0 ) {
					...
					/*if( !empty( $this->temp_path) && !empty($this->cv_file_name) ) {
						...
						if(!move_uploaded_file( $this->temp_path, $target_path)){
							...
							//$this->cv_experience_id = $db->insert_id();
							//$this->delete();
							return false;
						}
					}*/
...
#######################################################################################
	...
	public function save_cv_view( $client_id, $num_view ){
		...
		$sql = sprintf("INSERT INTO ".TBL_CLINT_CVVIEW." (id, fk_cv_id, fk_employer_id, view_for, created_at)
						 VALUES (NULL , '%s', '%s', '%s', '%s');",
							$db->escape_value( $this->cv_experience_id ),
...
########################################################################
	...
	public static function getCVEducationDetails( $id=0 ){
		global $database, $db;
		$sql = " SELECT * FROM ". self::$table_name;
	...
	public static function getCVEExperienceDetails( $exp_id=0 ){
		global $database, $db;
		$sql = " SELECT * FROM ". self::$table_name;
		$sql .= " WHERE exp_id= '".$db->escape_value($exp_id)."'";
		//$sql .= " LIMIT 1 ";
		//echo "--".$sql;exit;
		$result = self::find_by_sql_raw( $sql );
		if(is_array($result) && (count($result)>0))
	...
	public static function find_by_id( $exp_id=0,$pos_name="",$promo_date="" ){
		global $database, $db;
		$sql = " SELECT * FROM ". self::$table_name;
		$sql .= " WHERE experience_positions_details_id= ".$db->escape_value($exp_id)." AND position_title= '".$db->escape_value($pos_name)."' AND date_of_promotion='".$promo_date."'";
		$sql .= " LIMIT 1 ";
		//echo $sql;exit;
		$result = self::find_by_sql( $sql );
		return !empty($result) ? array_shift($result) : false;
	...
	public static function find_by_id_for_update( $id=0,$exp_id="",$str_position_name="",$strPromotionDate=""){
		global $database, $db;
		$sql = " SELECT * FROM ". self::$table_name;
		$sql .= " WHERE experience_positions_details_id	!= ".$db->escape_value($id). " AND exp_id= '".$db->escape_value($exp_id)."'"." AND position_title = '".$str_position_name."' AND date_of_promotion= '".$db->escape_value($strPromotionDate)."'";
		$sql .= " LIMIT 1 ";
		//echo $sql;exit;
		$result = self::find_by_sql( $sql );
		return !empty($result) ? array_shift($result) : false;
	...
	public static function find_compt_count_by_id( $id=0){
		...
		$sql = "SELECT COUNT(*) FROM ".self::$table_name;
		$sql .= " WHERE cv_experience_id= '".$db->escape_value($id)."'";
    	$result_set = $database->query($sql);
	  	$row = $database->fetch_array($result_set);
   	 	return array_shift($row);
	}
	...
	public function create() {
	  ...
	  $sql .= ") VALUES ('";
		...
		//echo "--".$sql;exit; 
	  if($database->query($sql)) {
	    $intLastInsertedId = $database->insert_id();
	...
	public function update() {
	  global $database, $db;
		...
		$sql .= " WHERE experience_positions_details_id=". $database->escape_value($this->experience_positions_details_id);
		//echo "--".$sql;exit; 
	  $database->query($sql);
	...
	public function deleteAll($intExpId = "") {
		global $database, $db;
	  $sql = "DELETE FROM ".self::$table_name;
	  $sql .= " WHERE exp_id=". $database->escape_value($intExpId);
	  $sql .= " LIMIT 1";
	  $database->query($sql);
	  return ($database->affected_rows() == 1) ? true : false;
	}
...
$cv_experience_position_setting 	= new CVExperiencePositionsSetting();
