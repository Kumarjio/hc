...
class CVCompetanciesSetting extends DatabaseObject{
	protected static $table_name = TBL_CVCOMPETANCIES;
	protected static $db_fields = array('cv_competancies_id', 'cv_id', 'cv_competancies_skill','cv_competancy_skill_year','cv_competancy_skill_month','cv_competancy_skill_from_date','cv_competancy_skill_to_date','cv_competancy_skill_created_by');
	
	public $cv_competancies_id;
	public $cv_id;
	public $cv_competancies_skill;
	public $cv_competancy_skill_year;
	public $cv_competancy_skill_month;
	public $cv_competancy_skill_from_date;
	public $cv_competancy_skill_to_date;
	public $cv_competancy_skill_created_by;
	...
	public function cv_review_by_employee(){
		global $database, $db;
		$sql=" SELECT * FROM ".self::$table_name;
		$sql.=" WHERE fk_employee_id=".(int)$this->fk_employee_id ." AND id=".$this->cv_awards_id;
		//echo $sql;
		$result = self::find_by_sql( $sql );
		return !empty($result) ? array_shift($result) : false;
	}
	...
	public function cv_review_by_employer(){
		global $database, $db;
		$sql=" SELECT * FROM ".self::$table_name;
		$sql.=" WHERE cv_status='Public' AND id=".$this->cv_awards_id;
	...
	public function update_no_views(){
		...
		$sql .= " SET no_views = no_views + 1 WHERE id=".$this->cv_awards_id;
		$database->query($sql);
	...
	public function make_defult(){
		...
		$sql.=" WHERE fk_employee_id='".$this->fk_employee_id."' AND id=".$this->cv_awards_id;
...
################################
	...
	public function delete_emp() {
	  ...
	  $sql .= " WHERE id=". $database->escape_value($this->cv_awards_id);
	  $sql .= " AND fk_employee_id='".$this->fk_employee_id."' ";
	  $sql .= " LIMIT 1";
	  $database->query($sql);
	  return ($database->affected_rows() == 1) ? true : false;
...
########################### SAVEING CV #####################################	
	...
	public function save() {
		...
		if( isset($this->cv_competancies_id) ){
			
			if( !is_numeric($this->cv_competancies_id) ){
				...
				if( sizeof($this->errors) == 0 ) {
					...
					/*if( !empty( $this->temp_path) && !empty($this->cv_file_name) ) {
						...
						if(!move_uploaded_file( $this->temp_path, $target_path)){
							...
							//$this->cv_awards_id = $db->insert_id();
							//$this->delete();
							return false;
						}
					}*/
...
#######################################################################################
	...
	public function save_cv_view( $client_id, $num_view ){
		...
		$sql = sprintf("INSERT INTO ".TBL_CLINT_CVVIEW." (id, fk_cv_id, fk_employer_id, view_for, created_at)
						 VALUES (NULL , '%s', '%s', '%s', '%s');",
							$db->escape_value( $this->cv_awards_id ),
...
########################################################################
	...
	public static function getCVCompetanciesDetails( $id=0 ){
		global $database, $db;
		$sql = " SELECT * FROM ". self::$table_name;
	...
	/*public static function fnConvertResultFormat( $array=array() ){
		global $database, $db;
		$arrResultant = array();
		
	}*/
	
	public static function find_by_id( $id=0,$skill_name="" ){
		global $database, $db;
		$sql = " SELECT * FROM ". self::$table_name;
		$sql .= " WHERE cv_id= ".$db->escape_value($id)." AND cv_competancies_skill= '".$db->escape_value($skill_name)."'";
		$sql .= " LIMIT 1 ";
		//echo $sql;exit;
		$result = self::find_by_sql( $sql );
		return !empty($result) ? array_shift($result) : false;
	...
	public static function find_compt_count_by_id( $id=0){
		...
		$sql = "SELECT COUNT(*) FROM ".self::$table_name;
		$sql .= " WHERE cv_competancies_id= '".$db->escape_value($id)."'";
    	$result_set = $database->query($sql);
	  	$row = $database->fetch_array($result_set);
   	 	return array_shift($row);
	}
	
	public static function find_by_id_for_update( $id=0,$skill_name="",$compt_id="" ){
		global $database, $db;
		$sql = " SELECT * FROM ". self::$table_name;
		$sql .= " WHERE cv_id= ".$db->escape_value($id)." AND cv_competancies_skill= '".$db->escape_value($skill_name)."'"." AND cv_competancies_id!= '".$db->escape_value($compt_id)."'";
		$sql .= " LIMIT 1 ";
		//echo $sql;exit;
		$result = self::find_by_sql( $sql );
		return !empty($result) ? array_shift($result) : false;
	...
	public function create() {
	  ...
	  $sql .= ") VALUES ('";
		...
		//echo $sql;exit;
	  if($database->query($sql)) {
	    $this->cv_competancies_id = $database->insert_id();
	    return $this->cv_competancies_id;
	  } else {
	    return false;
	  }
	}
	...
	public function update() {
	  global $database, $db;
		...
		$sql .= " WHERE cv_competancies_id=". $database->escape_value($this->cv_competancies_id);
...
$cv_competancies_setting 	= new CVCompetanciesSetting();
