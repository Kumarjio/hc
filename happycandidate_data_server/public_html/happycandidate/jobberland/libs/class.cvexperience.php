...
class CVExperienceSetting extends DatabaseObject{
	protected static $table_name = TBL_CVEXPERIENCE;
	protected static $db_fields = array('cv_experience_id', 'cv_id', 'cv_experience_current','cv_experience_company_name','cv_experience_company_location','cv_experience_company_job_title','cv_experience_company_responsibilities','cv_experience_company_accomplishments_impact');
	
	public $cv_experience_id;
	public $cv_id;
	public $cv_experience_current;
	public $cv_experience_company_name;
	public $cv_experience_company_location;
	public $cv_experience_company_job_title;
	public $cv_experience_company_responsibilities;
	public $cv_experience_company_accomplishments_impact;
...
########################### SAVEING CV #####################################	
	...
	public function save() {
		...
		if( isset($this->cv_experience_id) ){
			
			if( !is_numeric($this->cv_experience_id) ){
...
########################################################################
	...
	public static function getCVEExperienceDetails( $id=0 ){
		global $database, $db;
		$sql = " SELECT * FROM ". self::$table_name;
		$sql .= " WHERE cv_id= '".$db->escape_value($id)."'";
		//$sql .= " LIMIT 1 ";
	...
	public static function find_by_id( $id=0,$exp_type="",$exp_name="" ){
		global $database, $db;
		$sql = " SELECT * FROM ". self::$table_name;
		$sql .= " WHERE cv_id= ".$db->escape_value($id)." AND cv_experience_current= '".$db->escape_value($exp_type)."' AND cv_experience_company_name='".$exp_name."'";
		$sql .= " LIMIT 1 ";
	...
	public static function find_by_id_for_update( $id=0,$exp_type="",$exp_id="",$exp_name="" ){
		global $database, $db;
		$sql = " SELECT * FROM ". self::$table_name;
		$sql .= " WHERE cv_id= ".$db->escape_value($id)." AND cv_experience_current= '".$db->escape_value($exp_type)."'"." AND cv_experience_company_name = '".$exp_name."' AND cv_experience_id!= '".$db->escape_value($exp_id)."'";
		$sql .= " LIMIT 1 ";
		//echo $sql;exit;
		$result = self::find_by_sql( $sql );
		return !empty($result) ? array_shift($result) : false;
	...
	public function create() {
	  ...
	  $sql .= ") VALUES ('";
		...
		$sql .= "')";
	  if($database->query($sql)) {
	    $this->cv_experience_id = $database->insert_id();
	    return $this->cv_experience_id;
		//return true;
	  } else {
	    return false;
	  }
	...
	public function update() {
	  global $database, $db;
		...
		$sql .= " WHERE cv_experience_id=". $database->escape_value($this->cv_experience_id);
	  $database->query($sql);
