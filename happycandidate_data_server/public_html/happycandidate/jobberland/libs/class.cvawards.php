...
class CVAwardsSetting extends DatabaseObject{
	protected static $table_name = TBL_CVAWARDS;
	protected static $db_fields = array('cv_awards_id', 'cv_id', 'cv_awards','cv_awards_created_by');
...
########################### SAVEING CV #####################################	
	...
	public function save() {
		...
		if( isset($this->cv_awards_id) ){
			
			if( !is_numeric($this->cv_awards_id) ){
...
########################################################################
	...
	public static function getCVAwardsDetails( $id=0 ){
		global $database, $db;
		$sql = " SELECT * FROM ". self::$table_name;
	...
	public function create() {
	  ...
	  $sql .= ") VALUES ('";
		...
		$sql .= "')";
	  if($database->query($sql)) {
	    $this->cv_awards_id = $database->insert_id();
	    return true;
	  } else {
	    return false;
	  }
	...
	public function update() {
	  global $database, $db;
		...
		$sql .= " WHERE cv_awards_id=". $database->escape_value($this->cv_awards_id);
	  $database->query($sql);
	  return ($database->affected_rows() == 1) ? true : false;
	}
