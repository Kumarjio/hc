...
/***********************************************************************/
	...
	public static function fnUpdateDefaultCVCategory($arrFieldDetails = array(),$intCVId = ""){
		global $database, $db;
		...
		if(is_array($arrFieldDetails) && (count($arrFieldDetails)>0))
		{
			...
			$strUpdateFieldStatement = implode(" and ",$arrFieldContructor);
			$strSqlUpdateStatement = "UPDATE jobberland_cv_category SET "." ".$strUpdateFieldStatement." WHERE cv_id = '".$intCVId."'";
			//echo "---".$strSqlUpdateStatement;
			$database->query($strSqlUpdateStatement);
			return ($database->affected_rows() == 1) ? true : false;
		}
		else
		{
