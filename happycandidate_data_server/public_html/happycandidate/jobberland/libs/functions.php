...
function fnChangePasswordNotificationMessage($arrUserDetail = array())
{
	if(is_array($arrUserDetail) && (count($arrUserDetail)>0))
	{
		$strNotificationMessage = "Hello ".$arrUserDetail['name'].",<br><br>";
		$strNotificationMessage .= "Your Account Credentials has been changed, below are the updated credentials, Please use them to login.<br><br>";
		$strNotificationMessage .= "Username: ".$arrUserDetail['email']."<br>";
		$strNotificationMessage .= "Password: ".$arrUserDetail['newpassword']."<br><br>";
		
		
		$strNotificationMessage .= "Thanks"."<br>";
		$strNotificationMessage .= "HC";
...
function send_mail( $notes, $subject, $to, $from, $cv_file, $cv_letter ){
	...
	if ( !MAILER_HOST && MAILER_HOST == "" )
	{
		...
		*/
		...
		$mail->Host     = "ssl://smtp.gmail.com"; // SMTP server
		$mail->Port     = 465;
		$mail->Username = "rajendra.kandpal@redorangetechnologies.com"; // SMTP username
		$mail->Password = "rajendra12"; // SMTP password
