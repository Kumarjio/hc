...
class Employer extends DatabaseObject{
	...
	protected static $db_fields = array('id', 'extra_id','company_name',  'var_name', 'contact_name', 'site_link', 'company_logo', 
										...
										'date_register', 'last_login', 'actkey' , 'admin_comments', 'employer_status', 'is_active','hc_employer_id');
...
//get full name
 ...
 // Pass in $_FILE(['uploaded_file']) as an argument
  ...
  public static function change_key( $username ){
	...
	public function fnShortEmployerRegistration($arrEmployerDetail = array())
	{
		global $database, $db;
		
		if(is_array($arrEmployerDetail) && (count($arrEmployerDetail)>0))
		{
			$sql = " INSERT INTO ".self::$table_name;
			$sql .= " (username,email_address,hc_employer_id,employer_status)VALUES('".$arrEmployerDetail['uname']."','".$arrEmployerDetail['email']."','".$arrEmployerDetail['hcuid']."','Active')";
	...
	public function save() {
			...
			if( empty($this->company_name) ){
				...
				/** company_name **/
			if( empty($this->fname) ){
				$this->errors[] =  "Please Provide First Name";
			}
			
			if( empty($this->sname) ){
				$this->errors[] =  "Please Provide Surname";
			}
	  ...
	  // A new record won't have an id yet.
	   if( isset($this->id) ) {
		   ...
		   if( empty($this->state_province) ){
			   $this->errors[] = "Please select state/province/region from list";
		   }
	...
	public function active_hc_user(){
	  global $database, $db;
		$sql = " UPDATE user ";
		$sql .= " SET is_active = '1' 
				  WHERE id=".$this->hc_employer_id." LIMIT 1 ";
		//echo  $sql;exit; 
		$database->query($sql);
	  	return ($database->affected_rows() == 1) ? true : false;
  }
  ...
  public function deactive_hc_user(){
	  global $database, $db;
		$sql = " UPDATE user";
		$sql .= " SET is_active = '0', user_inactivation_date= '".date('Y-m-d H:i:s')."'  
				  WHERE id=".$this->hc_employer_id." LIMIT 1 ";
...
/***********************************************************************/
	...
	public static function find_portal_by_id($intPortalId = "")
	{
		if($intPortalId)
		{
			$strPortalQuery = "SELECT career_portal.* FROM career_portal WHERE career_portal_id = '".$intPortalId."'";
			$strPortalQueryExe = mysql_query($strPortalQuery);
			if($strPortalQueryExe)
			{
				$arrPortalDetail = mysql_fetch_array($strPortalQueryExe);
				/* print("<pre>");
				print_r($arrPortalDetail); */
				return $arrPortalDetail;
	...
	public static function find_portal_by_employer_id($intEmployerId = "")
	{
		if($intEmployerId)
		{
			$strEmployerPortalQuery = "SELECT career_portal.* FROM career_portal WHERE career_portal_provider_id = '".$intEmployerId."' ORDER BY career_portal_id DESC LIMIT 0,1";
	...
	public static function count_all_owner() {
	  	global $database, $db;
	  	$sql = "SELECT COUNT(*) FROM ".self::$table_name.", user WHERE jobberland_employer.hc_employer_id = user.id AND user.portal_id != '0'";
    	$result_set = $database->query($sql);
	  	$row = $database->fetch_array($result_set);
   	 	return array_shift($row);
	}
	...
	public function update($boolIsPassUpdate = "") {
	  global $database, $db;
		...
		if($boolIsPassUpdate)
		{
			$sql = " UPDATE user";
			//$sql .= " SET passwd = '".$new_pass."' 
			$sql .= " SET pass = '".$database->escape_value(md5(CAKESALTVALUE.$this->passwd))."' 
					  WHERE id = '".$this->hc_employer_id."' ";
			$sql .= " LIMIT 1";
			$database->query($sql);
