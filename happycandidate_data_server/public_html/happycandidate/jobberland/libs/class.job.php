...
class Job extends DatabaseObject{
	...
	protected static $db_fields = array('id', 'fk_employer_id', 'fk_hc_employer_id', 'fk_hc_portal_id', 'job_ref', 'var_name','job_title', 'job_description','job_requirenment','job_postion','post_code', 
	...
	public $fk_hc_employer_id=0;
	public $fk_hc_portal_id=0;
	public $job_ref;
	public $var_name;
	public $job_title;
	public $job_description;
	public $job_requirenment;
	public $job_postion;
	public $post_code;
	public $city;
	public $county;
...
/////	
	...
	public static function get_latest_job_as_per_time($strTime = "",$strEndTime = "")
	{
		global $database, $db;
		if($strTime && $strEndTime)
		{
			$sql = " SELECT jobberland_job.*,jobberland_job2category.* FROM ". self::$table_name.", jobberland_job2category";
			$sql .= " WHERE is_active='Y' AND job_status='approved'";
			$sql .= " AND id > 0 ";
			//$sql .= " AND created_at LIKE '".$strTime."%'";
			$sql .= " AND created_at >= '".$strTime."' AND created_at <= '".$strEndTime."'";
			$sql .= " AND nw_job_notification = '0'";
			$sql .= " AND jobberland_job2category.job_id = jobberland_job.id ";
...
############################# Saving and Updating ##########################	
	public function save() {
		...
		/*if( !isset($this->job_requirenment) || empty($this->job_requirenment) )
		{
			$this->errors[] = "Job Minimum Requirnment is Required";
		}*/
		...
		if( empty($this->state_province) || empty($this->post_code) ||  empty($this->country))
		{
...
/********************************* 	EMPLOYER *************************************/
	...
	public static function find_matched_cvs($arrMatch = array())
	{
		global $database;
		if(is_array($arrMatch) && (count($arrMatch)>0))
		{
			$keyword 	= $arrMatch['txt_keywords'];		
			$experience	= $arrMatch['txt_experience'];
			$education 	= $arrMatch['txt_education'];
			
			if( sizeof($arrMatch['txt_job_statu']) != 0 && isset($arrMatch['txt_job_statu'])){
				//$job_status=array();
				//foreach( $arrMatch['txt_job_statu'] as $key => $data ){
				//	$job_status[] = "look_job_status = '{$data}'";
				//}
				
				//$job_status = implode(' OR ', $job_status );
				$job_status = "look_job_status IN (". mysql_IN_values($arrMatch['txt_job_statu'])." ) ";
			}
			...
			if( !empty($_POST['txt_start_date_Month']) && 
				...
				$str_date 	= mktime(0,0,0, 
									 $arrMatch['txt_start_date_Month'], 
									 $arrMatch['txt_start_date_Day'], 
									 $arrMatch['txt_start_date_Year']);
				
				$str_date 	= date("Y-m-d H:i:s",$str_date);
			}
			
			//print_r($arrMatch['txt_location']);
			if( sizeof($arrMatch['txt_location']) != 0 && isset($arrMatch['txt_location'])){
				$loc2=array();
				foreach( $arrMatch['txt_location'] as $key => $data ){
					$loc2[] = "city = '{$data}'";
				}
				 $loc = implode(' OR ', $loc2 );
				 //$loc = "city IN (". mysql_IN_values( $arrMatch['txt_location'] )." ) ";
			}
			
			
			
			$salary		= $arrMatch['txt_salary'];

	//setup search query
			//$select = " SELECT jobberland_cv_detail.*,jobberland_employee.* ";
			$select = " SELECT jobberland_employee.id,jobberland_employee.hc_uid,jobberland_employee.hc_uid,jobberland_employee.hc_portal_id,jobberland_employee.email_address";
			
			$from = " FROM ".TBL_CV;
			
			$where = " WHERE jobberland_cv_detail.cv_status='Public' ";
			...
			if( sizeof($arrMatch['txt_category']) != 0 && isset($arrMatch['txt_category'])){
				$cat2=array();
				foreach( $arrMatch['txt_category'] as $key => $data ){
					$cat2[] = "cv_cat.category_id =".$data;
				}
				
				$cat = implode(' OR ', $cat2 );
				//$cat = "look_industries IN (". mysql_IN_values( $arrMatch['category'] )." ) ";
				$from .= ", ". TBL_CV_CAT . " AS cv_cat ";
				$where.= " AND ( ".$cat." ) ";
				$where.= " AND cv_cat.cv_id = jobberland_cv_detail.id";
			}
			
			if( !empty($experience) && $experience != "" ){
				$where.=" AND jobberland_cv_detail.year_experience ='$experience' ";
			}
			
			if( !empty($education) && $education != ""){
				$where.=" AND jobberland_cv_detail.highest_education ='$education' ";
			}
			...
			$from .= ",jobberland_employee,career_portal_candidate,candidate_settings";
			$where .= " AND jobberland_cv_detail.fk_employee_id = jobberland_employee.id AND jobberland_employee.hc_uid = career_portal_candidate.candidate_id AND candidate_settings.candidate_id = career_portal_candidate.candidate_id AND candidate_settings.is_job_notification = '1'";
			$group_by = " GROUP BY jobberland_cv_detail.id ";
			
			$sql = $select. $from . $where.$group_by;
			
			/*$fh = fopen("candidatequery.txt","w");
			$fh = fwrite($fh,"--".$sql);
			fclose($fh);*/
	...
	public static function new_job_notification($arrNotificationUser = array(),$arrNewJob = array())
	{
		if(is_array($arrNotificationUser) && (count($arrNotificationUser)>0))
		{
			$strNewJobAddEventNotificationUrl = "http://".$_SERVER['SERVER_NAME']."/happycandidate/portal/updatenewjobnotificationevent/";
			
			$fields_string = "notificationusers=".json_encode($arrNotificationUser)."&notificationsubjects=".json_encode($arrNewJob);
			
			$ch = curl_init();

			//set the url, number of POST vars, POST data
			curl_setopt($ch,CURLOPT_URL, $strNewJobAddEventNotificationUrl);
