...
class CVSummarySetting extends DatabaseObject{
	protected static $table_name = TBL_CVSUMMARY;
	protected static $db_fields = array('cv_summary_id', 'cv_id', 'cv_summary_type','cv_qualification_summary','cv_exe_summary','cv_summary_created_by');
	
	public $cv_summary_id;
	public $cv_id;
	public $cv_summary_type;
	public $cv_qualification_summary;
	public $cv_exe_summary;
	...
	public function cv_review_by_employee(){
		global $database, $db;
		$sql=" SELECT * FROM ".self::$table_name;
		$sql.=" WHERE fk_employee_id=".(int)$this->fk_employee_id ." AND id=".$this->cv_summary_id;
		//echo $sql;
		$result = self::find_by_sql( $sql );
		return !empty($result) ? array_shift($result) : false;
	}
	...
	public function cv_review_by_employer(){
		global $database, $db;
		$sql=" SELECT * FROM ".self::$table_name;
		$sql.=" WHERE cv_status='Public' AND id=".$this->cv_summary_id;
	...
	public function update_no_views(){
		...
		$sql .= " SET no_views = no_views + 1 WHERE id=".$this->cv_summary_id;
		$database->query($sql);
	...
	public function make_defult(){
		...
		$sql.=" WHERE fk_employee_id='".$this->fk_employee_id."' AND id=".$this->cv_summary_id;
...
################################
	...
	public function delete_emp() {
	  ...
	  $sql .= " WHERE id=". $database->escape_value($this->cv_summary_id);
	  $sql .= " AND fk_employee_id='".$this->fk_employee_id."' ";
	  $sql .= " LIMIT 1";
	  $database->query($sql);
	  return ($database->affected_rows() == 1) ? true : false;
...
########################### SAVEING CV #####################################	
	...
	public function save() {
		...
		if( isset($this->cv_summary_id) ){
			
			if( !is_numeric($this->cv_summary_id) ){
				...
				if( sizeof($this->errors) == 0 ) {
					...
					/*if( !empty( $this->temp_path) && !empty($this->cv_file_name) ) {
						...
						if(!move_uploaded_file( $this->temp_path, $target_path)){
							...
							//$this->cv_summary_id = $db->insert_id();
							//$this->delete();
							return false;
						}
					}*/
...
#######################################################################################
	...
	public function save_cv_view( $client_id, $num_view ){
		...
		$sql = sprintf("INSERT INTO ".TBL_CLINT_CVVIEW." (id, fk_cv_id, fk_employer_id, view_for, created_at)
						 VALUES (NULL , '%s', '%s', '%s', '%s');",
							$db->escape_value( $this->cv_summary_id ),
...
########################################################################
	...
	public static function getCVSummaryDetails( $id=0 ){
		global $database, $db;
		$sql = " SELECT * FROM ". self::$table_name;
	...
	public function create() {
	  ...
	  $sql .= ") VALUES ('";
		...
		$sql .= "')";
	  if($database->query($sql)) {
	    $this->cv_summary_id = $database->insert_id();
	    return true;
	  } else {
	    return false;
	  }
	...
	public function update() {
	  global $database, $db;
		...
		$sql .= " WHERE cv_summary_id=". $database->escape_value($this->cv_summary_id);
	  $database->query($sql);
	  return ($database->affected_rows() == 1) ? true : false;
	}
