...
class CVEducationSetting extends DatabaseObject{
	protected static $table_name = TBL_CVEDUCATION;
	protected static $db_fields = array('cv_education_id', 'cv_id', 'cv_education_highest','cv_education_school_uni_name','cv_education_location','cv_education_continue','cv_summary_created_by');
	
	public $cv_education_id;
	public $cv_id;
	public $cv_education_highest;
	public $cv_education_school_uni_name;
	public $cv_education_location;
	public $cv_education_continue;
	...
	public function cv_review_by_employee(){
		global $database, $db;
		$sql=" SELECT * FROM ".self::$table_name;
		$sql.=" WHERE fk_employee_id=".(int)$this->fk_employee_id ." AND id=".$this->cv_education_id;
		//echo $sql;
		$result = self::find_by_sql( $sql );
		return !empty($result) ? array_shift($result) : false;
	}
	...
	public function cv_review_by_employer(){
		global $database, $db;
		$sql=" SELECT * FROM ".self::$table_name;
		$sql.=" WHERE cv_status='Public' AND id=".$this->cv_education_id;
	...
	public function update_no_views(){
		...
		$sql .= " SET no_views = no_views + 1 WHERE id=".$this->cv_education_id;
		$database->query($sql);
	...
	public function make_defult(){
		...
		$sql.=" WHERE fk_employee_id='".$this->fk_employee_id."' AND id=".$this->cv_education_id;
...
################################
	...
	public function delete_emp() {
	  ...
	  $sql .= " WHERE id=". $database->escape_value($this->cv_education_id);
	  $sql .= " AND fk_employee_id='".$this->fk_employee_id."' ";
	  $sql .= " LIMIT 1";
	  $database->query($sql);
	  return ($database->affected_rows() == 1) ? true : false;
...
########################### SAVEING CV #####################################	
	...
	public function save() {
		...
		if( isset($this->cv_education_id) ){
			
			if( !is_numeric($this->cv_education_id) ){
				...
				if( sizeof($this->errors) == 0 ) {
					...
					/*if( !empty( $this->temp_path) && !empty($this->cv_file_name) ) {
						...
						if(!move_uploaded_file( $this->temp_path, $target_path)){
							...
							//$this->cv_education_id = $db->insert_id();
							//$this->delete();
							return false;
						}
					}*/
...
#######################################################################################
	...
	public function save_cv_view( $client_id, $num_view ){
		...
		$sql = sprintf("INSERT INTO ".TBL_CLINT_CVVIEW." (id, fk_cv_id, fk_employer_id, view_for, created_at)
						 VALUES (NULL , '%s', '%s', '%s', '%s');",
							$db->escape_value( $this->cv_education_id ),
...
########################################################################
	...
	public static function find_by_id( $id=0,$skill_name="" ){
		global $database, $db;
		$sql = " SELECT * FROM ". self::$table_name;
		$sql .= " WHERE cv_id= ".$db->escape_value($id)." AND cv_education_highest= '".$db->escape_value($skill_name)."'";
		$sql .= " LIMIT 1 ";
	...
	public static function find_compt_count_by_id( $id=0){
		...
		$sql = "SELECT COUNT(*) FROM ".self::$table_name;
		$sql .= " WHERE cv_education_id= '".$db->escape_value($id)."'";
    	$result_set = $database->query($sql);
	  	$row = $database->fetch_array($result_set);
   	 	return array_shift($row);
	}
	...
	public static function find_by_id_for_update( $id=0,$skill_name="",$compt_id="" ){
		global $database, $db;
		$sql = " SELECT * FROM ". self::$table_name;
		$sql .= " WHERE cv_id= ".$db->escape_value($id)." AND cv_education_highest= '".$db->escape_value($skill_name)."'"." AND cv_education_id!= '".$db->escape_value($compt_id)."'";
		$sql .= " LIMIT 1 ";
		//echo $sql;exit;
		$result = self::find_by_sql( $sql );
		return !empty($result) ? array_shift($result) : false;
	...
	public function create() {
	  ...
	  $sql .= ") VALUES ('";
		...
		$sql .= "')";
	  if($database->query($sql)) {
	    $this->cv_education_id = $database->insert_id();
		return $this->cv_education_id;
	    //return true;
	  } else {
	    return false;
	  }
	...
	public function update() {
	  global $database, $db;
		...
		$sql .= " WHERE cv_education_id=". $database->escape_value($this->cv_education_id);
		//echo $sql;exit;
	  $database->query($sql);
	  return ($database->affected_rows() == 1) ? true : false;
	}
...
$cv_summary_setting 	= new CVEducationSetting();
