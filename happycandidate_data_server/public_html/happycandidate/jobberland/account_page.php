<?php
	...
	$_SESSION['direct_to'] = BASE_URL."account/?portid=".$intPortId; 	
...
switch( $action ){
	...
	default:
		...
		$image =  Employee::getCandidateImage( $user_id );				if(count($image)>0)		{			$imgname = $image['candidate_picture'];			 $imgpath = $_SERVER["DOCUMENT_ROOT"]."/happycandidate/app/webroot/assets/candidateprofile/".$imgname;			 if(file_exists($imgpath))			 {				 $imgpath = $imgname;			 }			 else			 {				 $imgpath='';			 }		}		else		{			$imgpath = "";		}
		 $id 							= $user->id;		
		$email 							= $user->email_address;
		$username 						= $user->username;
		$_SESSION['account']['title'] 	= $title = $user->title;
		$fname 						 	= $user->fname;				$candidate_picture 				= $imgpath;
		$sname 							= $user->sname;
		...
		$smarty->assign( 'id', $id );				$smarty->assign( 'candidate_picture', $candidate_picture );		
		...
		if( isset($_POST['account_btn']) ){
			...
			if( $employee->save() ){
				...
				redirect_to( HCBASEURL."candidates/profile/".$intPortId);
				exit;
			}else{
				...
				$session->message ("<div class='success'>".$message."</div>");
				redirect_to( HCBASEURL."candidates/profile/".$intPortId);
				exit;
			}
		}
	...
	if ($_SESSION['loc']['stateprovince'] != '') {
		...
		$smarty->assign( 'intPortId', $intPortId );	
