...
function fnAssignMediaToContent(strImagePassUrl,imageId)
{
	...
	else
	{
		...
		else
		{
			...
			if(strImageString!="")
			{
				$("#txtEditorContent").Editor("setText", strImageString);
			}
...
//	alert(strContentId);
	...
	$.ajax({ 
			...
			success: function(data)
			{
				...
				{
					//alert(data.content)
				//	$('#tab-vendor-panel').removeClass('in active');
	...
	$.ajax({ 
			...
			success: function(data)
			{
				...
				if(data.status == "success")
				{
					...
					$('#tab-library-webinars').html(data.content);
...
function fnGetMediUploader()
{
	...
	if($('#service').length > 0)
	{
		...
		strUrl = strBaseUrl+"content/mediauploader/"+serviceId;
	}
	else
	{
		strUrl = strBaseUrl+"content/mediauploader";
	}
...
function fnShowVendorPopup(strVendorserviceUrl)
{
	window.open(strVendorserviceUrl,"_blank","width=1200,height=1200,location=0,titlebar=0,status=0,resizable=1,toolbar=0",false)
}
...
function fnGetSubstepservices(vendor_service_id)
{
	
	$.ajax({ 
			type: "GET",
			url: strBaseUrl+"vendorservices/vendorservicesasproduct/"+vendor_service_id,
			dataType: 'json',
			data:"",
			async:false,
			cache:false,
			success: function(data)
			{
				...
				if(data.status == "success")
				{
					//alert(data.content)
					$('.tabloader').hide();
					$('#tab-serviceproduct-panel').html(data.content);
