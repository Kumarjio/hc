 ...
 */
	...
	public function search($strKeywordSearch)
	{
		...
		$this->Paginator->settings = array(
			'Resources' => array(
				'limit' => 20,
				'conditions' => array('product_name'=>$strKeywordSearch,"product_type"=>"Services")
			)
		);
	...
	public function index() 
	{
		...
		$this->Paginator->settings = array(
			'Resources' => array(
				'limit' => 20,
				'conditions' => array('product_type' => 'Services'),
			)
		);
	...
	public function addimagemore()
	{
		...
		if($this->request->is('Post'))
		{
			...
			$boolContentOther = $this->ResourcesImages->updateAll(
								array('product_image_status'=>"'".$arrContentData['ResourcesImages']['product_image_status']."'"),
								array('product_images_id =' => $intImageId)
							);
			
			if($boolContentOther)
			{
				
				$strMessage = $compMessage->fnGenerateMessageBlock('Your image has been set successfully.','success');
	...
	public function add()
	{
		...
		if($this->request->is('Post'))
		{
			...
			$arrContentData['Resources']['product_name'] = $this->request->data['service_name'];
			$arrContentData['Resources']['product_access_link'] = $this->request->data['service_access_link'];
			$arrContentData['Resources']['product_cost'] = $this->request->data['service_cost'];
			//$arrContentData['Resources']['product_type'] = "Services";
			$arrContentData['Resources']['product_type'] = $this->request->data['service_type'];
			if($this->request->data['to_publish'])
			{
				$arrContentData['Resources']['product_publish_status'] = "1";
				$arrContentData['Content']['content_status']  = "published";
