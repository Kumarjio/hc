 ...
 */
class JsprocesssubstepsController extends AppController {
 ...
 */
	...
	public function edit($intProductId = "")
	{
		...
		if($intProductId)
		{
			...
			//print("<pre>");
			//print_r($arrProductContent);exit;
				...
				else
				{
					...
					else
					{
						...
						if($boolContentCreated)
						{
							...
							$boolUpdatedhas_content = $this->Categories->updateAll(							array('category_has_content'=>1),array('content_category_id =' => $intCreatedContentId)						);
	...
	public function search($strKeywordSearch)
	{
		...
		$strActionScript .= '<script type="text/javascript" src="'.Router::url('/js/jquery/jquery.tablesorter.js').'"></script>';
		$this->set('strActionScript',$strActionScript);
		$this->Session->write('strCancelUrl',Router::url(array('controller'=>'jsprocesssubsteps','action'=>'search'),true));
		$this->loadModel('Categories');
		...
		//exit;
		$this->Paginator->settings = array(
			'conditions' => array("content_category_name LIKE"=> "%".$strKeywordSearch."%",'job_search_category'=>'1','job_process_type'=>'Substeps'),
	...
	public function index() 
	{
		...
		$strActionScript .= '<script type="text/javascript" src="'.Router::url('/js/jquery/jquery.tablesorter.js').'"></script>';
		$this->set('strActionScript',$strActionScript);
		$this->Session->write('strCancelUrl',Router::url(array('controller'=>'jsprocesssubstepss','action'=>'index'),true));
		if($this->request->is('Post') && ($this->request->data['filter_on']))
		{
			$strProductFilterKeyword = $this->request->data['product_keyword'];
			
			$this->redirect(array('controller'=>'jsprocesssubsteps','action'=>'search',$strProductFilterKeyword));
		}
		...
		$this->Paginator->settings = array(
			'conditions' => array('job_search_category'=>'1','job_process_type'=>'Substeps'),
			//'order' => array('content_category_id' => 'DESC'),
			'limit' => 20
		);
