...
class PortalController extends AppController {
    ...
    public function beforeFilter() {
        parent::beforeFilter();
        $this->Auth->allow('registration', 'login', 'reset', 'jobsearch', 'forgotpassword', 'jobdetail', 'captcha', 'share', 'home', 'index1', 'index2', 'index3', 'index4', 'index5', 'getIndeedJobSerach', 'aboutus', 'privacypolicy', 'contactus', 'themehome', 'themepage', 'portalpreview', 'termsandprivacy');
    }
...
//        echo $this->layout;die;
        ...
        if ($intPortalExists) {
            ...
            $arrOffers = $this->Cpaoffers->find('all', array('conditions' => array('offer_status' => 'Active')));
//            echo '<pre>';print_r($arrOffers);die;
    ...
    public function getIndeedJobSerach($intPortalId) {
        ...
        if (count($indeedresults) > 0) {
            $arrLoggedUser = $this->Auth->user();
            $arrSearchJobData['candidate_id'] = $arrLoggedUser['candidate_id'];
            $arrSearchJobData['reference_id'] = '';
            $arrSearchJobData['career_portal_id'] = $intPortalId;
            $arrSearchJobData['action_type'] = "search job";
            $arrSearchJobData['feature'] = "CRM";
            $arrSearchJobData['date'] = date('Y-m-d h:i:s');
            $this->loadModel('JobStatistics');
            $this->JobStatistics->save($arrSearchJobData);
        }
        /** Job Click Candidate User Entry * */
    ...
    public function register($strPortalName = "") {
        $arrLoggedUser = $this->Auth->user();
        ...
        $this->loadModel('Portal');
        $intPortalExists = $this->Portal->find('count', array('conditions' => array('career_portal_name' => $strPortalName)));
        ...
        if ($intPortalExists) {
            ...
            /** Insert Visitors Records * */
            $sessionId = session_id();
            $ipAddress = $_SERVER['REMOTE_ADDR'];
            $filename = "./visitorssession/" . $sessionId . "_" . $ipAddress . ".txt";

            if (!file_exists($filename)) {
                $newFileName = './visitorssession/' . $sessionId . "_" . $ipAddress . ".txt";
                $myfile = fopen($newFileName, "w") or die("Unable to open file!");
                $txt = "";
                fwrite($myfile, $txt);
                fclose($myfile);

                $arrVisitedData = array();
                $arrVisitedData['ip_address'] = $ipAddress;
                $arrVisitedData['portal_id'] = $arrPortalDetail['0']['Portal']['career_portal_id'];
                $arrVisitedData['portal_name'] = strtolower($arrPortalDetail[0]['Portal']['career_portal_name']);
                $arrVisitedData['unique_value'] = $arrPortalDetail[0]['Portal']['career_portal_id']."".$ipAddress;
                $arrVisitedData['session_id'] = $sessionId;
                $arrVisitedData['registration'] = "No";
                $arrVisitedData['visited_date'] = date("Y-m-d h:i:s");

                $this->loadModel('PortalVisitors');
                $this->PortalVisitors->save($arrVisitedData);
            }
            /** Insert Visitors Records * */
            $this->loadModel('PortalTheme');
            $arrPortalThemeDetail = $this->PortalTheme->fnGetPortalThemeDetail($arrPortalDetail['0']['Portal']['career_portal_id']);

            $career_portal_theme_name = $arrPortalThemeDetail[0]['wizard_theme']['theme_name'];
            $this->set('arrPortalDetail', $arrPortalDetail);
            $this->set('strPortalName', strtolower($arrPortalDetail[0]['Portal']['career_portal_name']));
            $this->set('intPortalId', $arrPortalDetail['0']['Portal']['career_portal_id']);
            ...
            if ($intPortalId) {
                ...
                if (is_array($arrPortalDetail) && (count($arrPortalDetail) > 0)) {
                    ...
                    if (is_array($arrRegistrationFieldDetail) && (count($arrRegistrationFieldDetail) > 0)) {
                        ...
                        foreach ($arrRegistrationFieldDetail as $arrRegistrationField) {
                            ...
                            if (is_array($this->Session->read('current_user')) && (count($this->Session->read('current_user')) > 0)) {
                                $arrRegistrationData = $this->Session->read('current_user');

                                //echo "---".$arrRegistrationField['fields_table']['field_name'];

                                if (isset($arrRegistrationData['candidate_first_name'])) {

                                    if ($arrRegistrationField['fields_table']['field_name'] == 'first_name') {
                                        $arrCompleteRegistrationFieldDetail[$intForEachCount]['fields_table']['field_value'] = $arrRegistrationData['candidate_first_name'];
                                    }
                                }

                                if (isset($arrRegistrationData['candidate_last_name'])) {
                                    if ($arrRegistrationField['fields_table']['field_name'] == 'last_name') {
                                        $arrCompleteRegistrationFieldDetail[$intForEachCount]['fields_table']['field_value'] = $arrRegistrationData['candidate_last_name'];
                                    }
                                }


                                if (isset($arrRegistrationData['candidate_email'])) {
                                    if ($arrRegistrationField['fields_table']['field_name'] == 'email') {
                                        $arrCompleteRegistrationFieldDetail[$intForEachCount]['fields_table']['field_value'] = $arrRegistrationData['candidate_email'];
                                    }
                                }
                ...
                if ($this->request->is('post')) {
                    ...
                    if ($intCountCandidateExists) {
                        ...
                        $this->request->data['PortalUser']['portal_theme_name'] = addslashes(trim($career_portal_theme_name));
                        //print("<pre>");
                        ...
                        if (is_array($this->PortalUser->validate) && (count($this->PortalUser->validate) > 0)) {
                            if ($this->PortalUser->validates()) {
                                ...
                                if ($boolCandidateRegistered) {
                                    ...
                                    $boolUpdated = $this->PortalVisitors->updateAll(
							array('registration' => "'Yes'"),
							array('session_id' => $sessionId,'ip_address' => $ipAddress)
						);
                                    ...
                                    $this->request->data['Candidate']['portal_theme_name'] = addslashes(trim($career_portal_theme_name));
...
//        echo "--".$this->layout;die;
        ...
        $intPortalExists = $this->Portal->find('count', array(
//            'conditions' => array('career_portal_name' => $strPortalName)
            'conditions' => array('career_portal_name' => $strPortalName,'career_portal_published'=>'1','career_portal_approved'=>'1')
        ));
        ...
        if ($intPortalExists) {
            ...
            if (!file_exists($filename)) {
                ...
                $arrVisitedData['portal_id'] = $arrPortalDetail[0]['Portal']['career_portal_id'];
                $arrVisitedData['unique_value'] = $arrPortalDetail[0]['Portal']['career_portal_id']."".$ipAddress;
                $arrVisitedData['portal_name'] = strtolower($strPortalName);
                $arrVisitedData['session_id'] = $sessionId;
                $arrVisitedData['registration'] = "No";
                $arrVisitedData['visited_date'] = date("Y-m-d h:i:s");
            ...
            //echo '<pre>';print_r($arrPortalDetail);exit();			
            ...
            //echo"<pre>"; print_r($arrMenuDetail); exit;
            $this->set('arrPortalMenuDetail', $arrMenuDetail);

            // courses detail
            $compLmsBridge = $this->Components->load('LmsBridge');
            $arrCourseDetails = $compLmsBridge->fnGetPortalCourses($arrPortalDetail['0']['Portal']['career_portal_id']);
            //print("<pre>"); print_r($arrCourseDetails); exit;
            ...
            //print("<pre>"); print_r($arrPageList); exit;
            ...
            //print("<pre>"); print_r($arrPortalHomePageDetail); exit;
            $this->set('arrPortalPageDetail', $arrPortalHomePageDetail);
        }else{
            
            $strRedirectUrl = Router::url(array('controller' => 'portal', 'action' => 'home', "monster"), true);
            
            header("HTTP/1.1 301 Moved Permanently");
            header("Location: $strRedirectUrl");
            exit();
            $this->layout = "error-page";
//            $this->Session->setFlash("Sorry career portal not published.please contact to system adminstrator.", 'default', array('class' => 'danger'),'danger');
        }
    }
...
//        echo '<pre>';print_r($arrPortalThemeDetail); exit();
        ...
        $strHomeEditorLink = Router::url('/', true) . "portal/home/" . $arrPortalThemeDetail['Portal']['career_portal_name'];
        $this->set('strHomeEditorLink', $strHomeEditorLink);
        $strRegister = Router::url(array('controller' => 'portal', 'action' => 'register', $arrPortalThemeDetail["Portal"]["career_portal_name"]), true);
        $this->set('strRegister', $strRegister);
        ...
        if (is_array($arrPageDetail)) {
            ...
            if (is_array($arrPortalDetail) && (count($arrPortalDetail) > 0)) {
                ...
                $this->loadModel('PortalTheme');
                $arrPortalThemeDetail = $this->PortalTheme->fnLoadPortalThemeDetail($arrPageDetail['1']);
                //echo "<pre>";print_r($arrPortalThemeDetail);
                ...
                if (strpos($arrPortalPageDetail[0]['PortalPages']['career_portal_page_tittle'], "Contact") !== False) {
                    ...
                    //print("<pre>");
                    //print_r($arrContactFormDetail); exit;
                    if (is_array($arrContactFormDetail) && (count($arrContactFormDetail) > 0)) {
                        $this->loadModel('ContactFormFields');
                        //$arrContactFormFields = $this->ContactFormFields->find('all',array('conditions'=>array('career_portal_contact_us_form_id'=>$arrContactFormDetail[0]['PortalContactForm']['career_portal_contact_us_form_id'])));
                        $arrContactFormFields = $this->ContactFormFields->fnGetAllFields($arrContactFormDetail[0]['PortalContactForm']['career_portal_contact_us_form_id']);
                        //print("<pre>");
                        //print_r($arrContactFormFields); exit;
                        if (is_array($arrContactFormFields) && (count($arrContactFormFields) > 0)) {
                            $intContactFormFieldCount = 0;
                            foreach ($arrContactFormFields as $arrContactFFields) {
                                $arrContactFormValidations = $this->ContactFormFields->fnGetAllFieldValidation($arrContactFFields['fields_table']['field_id']);
    ...
    public function getjobdetail($intPortalId = "", $intJobId = "") {
        if ($intPortalId) {
            ...
            /** Job Click Candidate User Entry * */
            $arrClickJobData['candidate_id'] = $candidate_id;
            $arrClickJobData['reference_id'] = $intJobId;
            $arrClickJobData['career_portal_id'] = $intPortalId;
            $arrClickJobData['action_type'] = "job click";
            $arrClickJobData['feature'] = "job boards";
            $arrClickJobData['action_date'] = date('Y-m-d h:i:s');
            $this->loadModel('JobStatistics');
            $this->JobStatistics->save($arrClickJobData);
        ...
        if ($intPortalId) {
            ...
            if (isset($_POST['submit'])) {
                ...
                if ($cntapply > 0) {
                    ...
                    /** Job Click Candidate User Entry * */
                    $arrJobApplyData['candidate_id'] = $intJobId;
                    $arrJobApplyData['reference_id'] = $this->JobsApplied->getLastInsertID();
                    $arrJobApplyData['action_type'] = "job apply";
                    $arrJobApplyData['feature'] = "CRM";
                    $arrJobApplyData['date'] = date('Y-m-d h:i:s');
                    $this->loadModel('JobStatistics');
                    $this->JobStatistics->save($arrJobApplyData);
                    /** Job Click Candidate User Entry * */
...
//        Configure::write('debug', 2);
        ...
        if ($intPortalId) {
            ...
            if ($arrLoggedUser['type'] == '1') {
//                $this->loadModel('Candidate');
//                $this->Candidate->fnDeleteCandidateData($arrLoggedUser['candidate_id']);
                $this->Auth->logout();
...
//            $this->set('isvisited', "1");
//            $this->set('arrMixPanelVisitedData', $arrVisitedData);
            }
            /** Check Visitors * */
            $this->loadModel('TopMenu');
            $arrMenuDetail = $this->TopMenu->find('all', array("order" => array('career_portal_menu_order' => 'ASC'), 'conditions' => array('career_portal_id' => $arrPortalDetail[0]['Portal']['career_portal_id'])));
            ...
            if ($this->request->is('post')) {
                ...
                if ($this->Candidate->validates()) {
                    if ($this->Auth->login()) {
                        $arrLoggedInUser = $this->Auth->user();
                        //update last login candidate
                        $lastLoginDateTime = date('Y-m-d h:i:s');
                        $this->Candidate->updateAll(array('last_login_date' => "'$lastLoginDateTime'"), array('candidate_id' => $arrLoggedInUser['candidate_id']));
                        //update last login candidate
...
//        echo "ID contact".$intPortalId; exit;
        $this->autoRender = false;
        if ($intPortalId) {
            $this->loadModel('Portal');
            $arrPortalDetail = $this->Portal->find('all', array(
            ...
            if (is_array($arrPortalDetail) && (count($arrPortalDetail) > 0)) {
                ...
                if ($this->request->is('post')) {
                    ...
                    if ($intContactFormId) {
                        ...
                        array_pop($arrContactFormFields);
                        //echo "<pre>"; print_r($arrContactFormFields);exit;
                        ...
                        if (is_array($arrContactFormFields) && (count($arrContactFormFields) > 0)) {
                            foreach ($arrContactFormFields as $arrContactFieldDetail) {
                                ...
                                //echo "<pre>"; print_r($arrContactValidations);exit;
                                ...
                                //echo "<pre>"; print_r($arrContactFieldDetail);exit;
                    ...
                    //print("<pre>");print_r($this->Contactus);exit;
                    //print('<pre>');print_r($arrContactUs);exit;
                    if (!isset($this->Captcha)) {
                        //if Component was not loaded throug $components array()
                        //$this->Captcha = $this->Components->load('Captcha'); //load it
                    }
                    ...
                    //$this->Contactus->setCaptcha("ABCD");
                    //print("<pre>");print_r($arrContactUs);exit;
...
//                               echo '<pre>';print_r($arrUserDetail);die;
                        ...
                        //echo "In validates ELSE";
                        $arrContactUsErrors = $this->Contactus->invalidFields();
                        //print("<pre>");print_r($arrContactUsErrors);//exit;
...
//            $this->set('arrMixPanelVisitedData', $arrVisitedData);
    ...
    //PORTAL EMPLOYER THEME FRONTEND
    public function portalpreview($strPortalName = "") {
//        Configure::write('debug', 2);
        $this->loadModel('Portal');
        $intPortalExists = $this->Portal->find('count', array(
            'conditions' => array('career_portal_name' => $strPortalName,'career_portal_published'=>'1')
        ));
...
//            $this->set('arrMixPanelVisitedData', $arrVisitedData);
            ...
            if ($this->params['action'] == "portalpreview") {
                $this->layout = $arrPortalThemeDetail['Wiztheme']['theme_name'] . '-' . $arrPortalThemeDetail['Wiztheme']['theme_color'] . '-FRONTEND';
            }
            if ($this->params['action'] == "themepage") {
                $this->layout = $arrPortalThemeDetail['Wiztheme']['theme_name'] . '-' . $arrPortalThemeDetail['Wiztheme']['theme_color'] . '-FRONTENDPAGE';
