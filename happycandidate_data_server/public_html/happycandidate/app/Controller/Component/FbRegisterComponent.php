...
//        } else {
//            $strRedirctUri = "http://".$_SERVER['HTTP_HOST'] . $this->Controller->params->here;
//            $loginUrl = $facebook->getLoginUrl(array('scope' => 'email,read_stream, publish_stream, user_birthday, user_location, user_work_history, user_hometown, user_photos', 'redirect_uri' => $strRedirctUri,'display' => 'popup'));
//            $this->Controller->redirect($loginUrl);
//        }
//    }
    ...
    public function fnGetFbUserDetails($intCareerPortalId = "", $boolForLogin = "",$type="") {
//        echo '<pre>';print_r($intCareerPortalId);die;
        if ($_REQUEST['user_email'] == 'undefined') {
            $strRedirctUri = "http://" . $_SERVER['HTTP_HOST'] . $this->Controller->params->here;
            $this->Controller->redirect($strRedirctUri);
        } else {
            $modelCandidateDetail = ClassRegistry::init('Candidate');
            $arrUserExists = $modelCandidateDetail->find('all', array('conditions' => array('candidate_email' => $_REQUEST['user_email'])));
                
           
            $users1 = split("@", $_REQUEST['user_email']);
            $username = $users1[0];
            $full_name = explode(" ", $_REQUEST['fb_name']);
            $arrUserArray['fb_id'] = $_REQUEST['fb_id'];
            $arrUserArray['candidate_first_name'] = $full_name[0];
            $arrUserArray['candidate_last_name'] = $full_name[1];
            $arrUserArray['candidate_username'] = $username;
            $arrUserArray['candidate_email'] = $_REQUEST['user_email'];
            $arrUserArray['candidate_password'] = AuthComponent::password($username);
            $arrUserArray['candidate_password_decrypted'] = $username;
            $arrUserArray['user_type'] = "2";
            $arrUserArray['type'] = "1";
            $arrUserArray['candidate_confirmation_mail_sent'] = "1";
            $arrUserArray['candidate_confirmed'] = "1";
            $arrUserArray['candidate_is_active'] = "1";
            $arrUserArray['career_portal_id'] = $intCareerPortalId;
            
            if(count($arrUserExists) <= 0 ){
                $modelPortalCandidateDetail = ClassRegistry::init('PortalUser');
                $modelPortalCandidateDetail->save($arrUserArray);
            }
            if ($arrUserArray['candidate_email'] != '') {
                
                
                if ($arrUserExists[0]['Candidate']) {
                    $this->Session->write('current_user', $arrUserExists[0]['Candidate']);
                    if($type == 'login'){
                        $strRedirectUrl = Router::url(array('controller' => 'portal', 'action' => 'login',$intCareerPortalId), true);
                        $this->Controller->redirect($strRedirectUrl);
                    }else{
                        $strRedirectUrl = Router::url(array('controller' => 'portal', 'action' => 'register',$intCareerPortalId), true);
                        $this->Controller->redirect($strRedirectUrl);
                    }
                }else{
                    $arrResultSet = array();
