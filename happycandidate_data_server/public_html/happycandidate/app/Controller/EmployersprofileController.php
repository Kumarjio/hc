...
class EmployersprofileController extends AppController {
    ...
    public function index() {  //echo "<pre>"; print_r($this->request->data); exit;
            ...
            if ($this->User->validates()) {
                ...
                if ($this->Employer->validates()) {
                    ...
                    $boolUpdated = $this->User->updateAll(
                            array('User.username' => "'" . $arrEmployerBasicPostedData['username'] . "'", 'User.email' => "'" . $arrEmployerBasicPostedData['email'] . "'"), array('User.id =' => $arrLoggedUserDetails['id'])
                    );
                    if ($boolUpdated) {
                        /* print("<pre>");
                          print_r($arrEmployerDetailPostedData); */
    ...
    public function paymentdetails() {
         $this->loadModel('Employer');
        $arrCurrentUser = $this->Auth->user();
        $arrEmployerDetail = $this->Employer->find('all', array('conditions' => array('employer_id' => $arrCurrentUser['id'])));
        $this->set('arrEmployerDetail', $arrEmployerDetail);
        $this->set('arrLoggedInUserDetail', $arrCurrentUser);
       
        if ($this->request->is('Post')) {
            $arrPaymentDetailsEntered = $this->Employer->updateAll(array(
                            'Employer.payout_to' => "'" . $this->request->data['payoutto'] . "'",
                            'Employer.tax_id' => "'" . $this->request->data['taxid'] . "'",
                            'Employer.bank_account_number' => "'" . $this->request->data['bankaccountnumber'] . "'",
                            'Employer.bank_routing_number' => "'" . $this->request->data['bankroutingnumber'] . "'",
                                ), array('Employer.employer_id =' => $arrCurrentUser['id'])
                        );
            if ($arrPaymentDetailsEntered) {
                $strMssg = "Your Payment Details were saved successfully.";
                $strMssgClass = "alert-success";
            } else {
                $strMssg = "Some issues, Please try again.";
                $strMssgClass = "alert-error";
