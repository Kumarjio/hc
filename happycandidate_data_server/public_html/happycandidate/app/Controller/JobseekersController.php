 ...
 */
	...
	{	
		...
		if($this->request->is('Post') && ($this->request->data['filter_on']))
		{
			...
			$this->redirect(array('controller'=>'users','action'=>'search',$strProductFilterKeyword));
		}
						
		$this->paginate = array( 'fields' => array('employee.*','portal.career_portal_name'), 'joins'=>array(array('table' => 'career_portal','alias' => 'portal','type' => 'inner','recursive' => -1,'conditions'=> array('portal.career_portal_id = employee.hc_portal_id'))),'conditions' => $condition , 'limit' => 10);			
