 ...
 */
	...
	public function search($strKeywordSearch) {
        ...
        if($this->request->data['columnName'] == '' && $this->request->data['sort'] == ''){
            ...
            $this->Session->write('vendorsearchcolumn',$columnName);
            $this->Session->write('vendorsearchsort',$sort);
            $this->Session->write('vendortype','search');
        }
        ...
        if ($columnName != '' && $sort != '') {
            ...
            $this->Paginator->settings = array(
                'conditions' => array('product_name' => $strKeywordSearch, 'column_name' => $columnName, 'order' => $sort),
                'limit' => 20
            );
                    ...
                    if ($intVendorServiceType == "Product") {
                        ...
                        if (is_array($arrVendorDetail) && count($arrVendorDetail) > 0) {
                            foreach ($arrVendorDetail as $arrVendDetail) {
                                ...
                                $arrProductContentList[$intProductCount]['vendors']['vendor_name'] = $arrVendDetail['Vendors']['vendor_name'];
//							$arrProductContentList[$intProductCount]['vendors']['vendor_name'] = $arrVendDetail['Vendors']['vendor_first_name']." ".$arrVendDetail['Vendors']['vendor_last_name'];
            ...
            $strContactLocationHtml = $view->element('sort_vendorservices_list');
            $strPaginationHtml = $view->element('sort_vendor_pagination',array('column_name'=>$columnName,'sort'=>$sort,'strKeywordSearch'=>$strKeywordSearch));
            ...
            $this->Paginator->settings = array(
                'Vendorservice' => array(
                    'conditions' => array('product_name' => $strKeywordSearch,'column_name'=>$ColumnName[0],'order'=>$Sort[0]),
                    'limit' => 20
                )
            );
...
//        $strActionScript .= '<script type="text/javascript" src="' . Router::url('/js/jquery/jquery.tablesorter.widgets.js') . '"></script>';
        ...
        if($this->request->data['columnName'] == '' && $this->request->data['sort'] == ''){
            ...
            $columnName = $this->request->data['columnName'];
            $sort = $this->request->data['sort'];
            $this->Session->write('vendorcolumn',$columnName);
            $this->Session->write('vendorsort',$sort);
            $this->Session->write('vendortype','index');
        }
        ...
        if ($columnName != '' && $sort != '') {
            ...
            $this->Vendorservice->recursive = 0;
            $this->Paginator->settings = array(
                'conditions' => array('column_name' => $columnName, 'order' => $sort),
                'limit' => 20
//                    'Vendorservice' => array(
//                    'conditions' => array('column_name'=>$columnName,'order'=>$sort),
//                    'limit' => 20
...
//							$arrProductContentList[$intProductCount]['vendors']['vendor_name'] = $arrVendDetail['Vendors']['vendor_first_name']." ".$arrVendDetail['Vendors']['vendor_last_name'];
            ...
            $strContactLocationHtml = $view->element('sort_vendorservices_list',array('column_name'=>$columnName,'sort'=>$sort));
            $strPaginationHtml = $view->element('sort_vendor_pagination',array('column_name'=>$columnName,'sort'=>$sort));
            if ($arrProductContentList) {
                $arrResponse['status'] = "success";
                $arrResponse['content'] = $strContactLocationHtml;
                $arrResponse['pagidiv'] = $strPaginationHtml;
            ...
            $this->Vendorservice->recursive = 0;
            $this->Paginator->settings = array(
                'conditions' => array('column_name'=> isset($ColumnName[0]) ? $ColumnName[0] : 'vendor_name','order'=> isset($Sort[0]) ? $Sort[0] : 'ASC'),
                'limit' => 20
            );
            $arrProductContentList = $this->Paginator->paginate('Vendorservice');
            if (is_array($arrProductContentList) && (count($arrProductContentList) > 0)) {
        ...
        public function edit($intProductId = "")
	{
		if($intProductId)
		{
			...
			$arrProductContent = $this->Vendorservice->find('all',array('conditions'=>array('vendor_service_id'=>$intProductId)));
			...
			$arrResourcesDetails = $this->Resources->find('all',array('conditions'=>array('product_type'=>array('Services'),'product_publish_status'=>'1', 'product_parent' => '')));
                        ...
                        $this->loadModel('Portal');
                        $arrPortalVendors = $this->Portal->find('all',array('fields'=>array('career_portal_id','career_portal_name')));
//                                    
//			print("<pre>");
//			print_r($arrPortalVendors);
//			exit;
			$arrVendorServiceDetail['Services'] = $arrResourcesDetails;
			$this->set('arrVendorServiceDetail',$arrVendorServiceDetail);
			$this->set('arrPortalVendors',$arrPortalVendors);
			}
	}
	...
	public function getservicedetails($intServiceId = "")
	{
		...
		if($intServiceId)
		{
			...
			{
				$arrResponse['status'] = 'success';
                                if($arrResources[0]['Resources']['discount_cost'] == '' || $arrResources[0]['Resources']['discount_cost'] == '0.00'){
                                    $arrResponse['cost'] = $arrResources[0]['Resources']['product_cost'];
                                }else{
                                    $arrResponse['cost'] = $arrResources[0]['Resources']['discount_cost'];
                                }
			}
			else
			{
...
//		$intUserType = $this->request->data['user_type'];
//		
//		$compMessage = $this->Components->load('Message');
//		
//		if($strType)
...
//                                $this->loadModel('Vendors');
//                                $arrProductContent = $this->Vendorservice->find('all',array('conditions'=>array('vendor_service_id'=>$intProductId)));
//				$strFileUploaderHtml = $view->element('servicevendors', array('arrProductContent'=>$arrProductContent));
//			}
//			else if($strType == "Course")
//			{
//                                $this->loadModel('Vendors');
//                                $arrProductContent = $this->Vendorservice->find('all',array('conditions'=>array('vendor_service_id'=>$intProductId)));
//                        
//				$strFileUploaderHtml = $view->element('coursevendors', array('arrProductContent'=>$arrProductContent));
//			}
//			else
//			{
//                                if($intUserType == 'Vendor'){
...
//                                    $this->loadModel('Vendors');
//                                    $arrProductContent = $this->Vendorservice->find('all',array('conditions'=>array('vendor_service_id'=>$intProductId)));
//                                    $strFileUploaderHtml = $view->element('productvendors', array('arrProductContent'=>$arrProductContent));
//                                }
//				
//			}
//			
...
//	}
		...
		if($strType)
		{
			...
			else
			{
                            ...
                            if(is_array($arrVendorDetails) && (count($arrVendorDetails)>0))
                            {
                                    ...
                                    foreach($arrVendorDetails as $arrVendor)
                                    {
                                            $arrVendorDetail[$intVendorCnt]['Vendors']['vendor_id'] = $arrVendor['Owner']['employer_id'];
                                            $arrVendorDetail[$intVendorCnt]['Vendors']['vendor_first_name'] = $arrVendor['Owner']['employer_user_fname'];
                                            $arrVendorDetail[$intVendorCnt]['Vendors']['vendor_last_name'] = $arrVendor['Owner']['employer_user_lname'];
                                            $intVendorCnt++;
                                    }
                                    $arrVendorDetails = $arrVendorDetail;
                            }  
			...
			$arrVendorServiceDetail['Vendors'] = $arrVendorDetails;
			$this->loadModel('Resources');
			$arrResourcesDetails = $this->Resources->find('all',array('conditions'=>array('product_type like'=> '%'.$strType.'%','product_parent'=>'')));
	...
	public function add($intProductId) {
        ...
        if ($this->request->is('Post')) {
            ...
            $vendor_id = trim($this->request->data['vendor']);
            $portalId = array();
            $portalIds = trim($this->request->data['portalId'], ",");
            $portalId = explode(",", $portalIds);
            ...
            $strDefaultCate = '0';
                ...
                $arrContentData['Vendorservice']['vendor_id'] = addslashes($vendor_id);
                $arrContentData['Vendorservice']['portal_id'] = addslashes($portalId);
                $arrContentData['Vendorservice']['user_type'] = addslashes($this->request->data['user_type']);
            ...
            if ($intEditModeId) {
                ...
                if ($boolContentUpdated) {
                    ...
                    $strMessage = $compMessage->fnGenerateMessageBlock($strForMessage, 'success');
                    $arrResponseData['status'] = 'success';
                    $arrResponseData['createdid'] = $intEditModeId;
                    $arrResponseData['message'] = $strMessage;
                ...
                $intContentExists = $this->Vendorservice->find('count', array(
                    'conditions' => array("service_id" => $arrContentData['Vendorservice']['service_id'])
//                    'conditions' => array('vendor_id' => $arrContentData['Vendorservice']['vendor_id'], "service_id" => $arrContentData['Vendorservice']['service_id'])
                ));
                if ($intContentExists) {
                    $compMessage = $this->Components->load('Message');
                    $strMessage = $compMessage->fnGenerateMessageBlock('These details are already present', 'error');
                    $arrResponseData['status'] = 'fail';
                    $arrResponseData['message'] = $strMessage;
                    ...
                    if (is_array($arrVendDet) && (count($arrVendDet) > 0)) {
                        if (is_array($arrResourcesDet) && (count($arrResourcesDet) > 0)) {
                            ...
                            if ($arrCourseCreated['status'] == "success") {
                                $boolContentCreated = $this->Vendorservice->saveAll($arrContentData);
                                if ($boolContentCreated) {
        ...
        $this->loadModel('Resources');
        $arrResourcesDetails = $this->Resources->find('all', array('conditions' => array('product_type' => array('Services'), 'product_publish_status' => '1', 'product_parent' => '')));
//        echo '<pre>';print_r($arrResourcesDetails);die;
        $arrVendorServiceDetail['Services'] = $arrResourcesDetails;
...
//		if(count($phaseid)> 0 && $vendor_service_id !='')		
		{			
...
//			$arrResponse['message'] = "Substeps are assigned vendor services successfully";		
                        $compMessage = $this->Components->load('Message');
                        $strForMessage = "Substeps are assigned vendor services successfully";						
                        $strMessage = $compMessage->fnGenerateMessageBlock($strForMessage,'success');
                        $arrResponse['status'] = 'success';
                        $arrResponse['message'] = $strMessage;
		}else{		
                    $compMessage = $this->Components->load('Message');
                    $strForMessage = "Something wrong happen please check";						
