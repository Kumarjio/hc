 ...
 */
class VendorservicehosterController extends AppController {
 ...
 */
	public $uses = array();
	public $layout = "admin";
	public $name = 'Vendorservicehoster';
 ...
 */
	...
	public function productdelete($intProductId)
	{
		...
		if($intProductId)
		{
			...
			$intCorrectProductId = $this->Vendorservice->find('count',array('conditions'=>array('vendor_service_id'=>$intProductId)));
			if($intCorrectProductId)
			{
	...
	public function search($strKeywordSearch)
	{
		...
		if(is_array($arrProductContentList) && (count($arrProductContentList)>0))
		{
			...
			foreach($arrProductContentList as $arrProductContent)
			{
				...
				$arrVendorDetail = $this->Vendors->find('all',array('controller'=>array('vendor_id'=>$arrProductContent['Vendorservice']['vendor_id'])));
				...
				$this->loadModel('Resources');
				$arrResourcesDetail = $this->Resources->find('all',array('controller'=>array('productd_id'=>$arrProductContent['Vendorservice']['service_id'])));
	...
	public function index($intPortalId = "",$strVendorService = "") 
	{
		...
		if($intPortalId)
		{
			...
			$arrVendorAccountUser = $this->Vendorcandidateacc->find('all',array('conditions'=>array('candidate_id'=>$arrLoggedUser['candidate_id'],'vendor_id'=>'1','account_id !='=>NULL)));
			...
			else
			{
				...
				$strPos = strpos($arrCandidateDetail[0]['Candidate']['candidate_email'],"@");
				$strNewMail = substr_replace($arrCandidateDetail[0]['Candidate']['candidate_email'],"@hc-test.com",$strPos,strlen($arrCandidateDetail[0]['Candidate']['candidate_email']));
				//$strNewMail = strtolower($arrCandidateDetail[0]['Candidate']['candidate_first_name'])."_".strtolower($arrCandidateDetail[0]['Candidate']['candidate_last_name'])."@hc-test.com";
				//$strNewMail = strtolower("rajendrakp")."@hc-test.com";
				
				
				$arrVendorCandidateRegDetails = array();
				$arrVendorCandidateRegDetails['fname'] = $arrCandidateDetail[0]['Candidate']['candidate_first_name'];
				$arrVendorCandidateRegDetails['lname'] = $arrCandidateDetail[0]['Candidate']['candidate_last_name'];
				$arrVendorCandidateRegDetails['email'] = $strNewMail;
				...
				if(is_array($arrUserDetail) && count($arrUserDetail)>0)
				{
					if($arrUserDetail['result'] == "success")
					{
						...
						$arrVendorCandidate['Vendorcandidateacc']['vendor_id'] = "1";
