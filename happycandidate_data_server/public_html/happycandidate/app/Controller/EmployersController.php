...
class EmployersController extends AppController {
    ...
    public function beforeFilter() {
        ...
        parent::beforeFilter();
        $this->Auth->allow('index', 'captcha', 'contactus', 'subscriptionactivation', 'termsandprivacy');
    }
...
//        $this->layout = "employers";
        ...
        if ($strEmail) {
            ...
            if ($arrUserDetail[0]['User']['user_confirmed']) {
                ...
                $this->Session->setFlash('Your account is not confirmed yet, Please click the confirmation link sent to your email at time of registration.', 'default', array('class' => 'success'));
            }
...
//        echo $this->layout; exit();
        ...
        $arrEmployerDomain = $this->Employerdomain->find('first', array('conditions' => array('employer_id' => $arrLoggedUser['id']), 'order' => 'employer_domain_id DESC'));

        if ($arrAdminDetails[0]['is_wizard'] == "0") {
            //echo 'in wiz';//exit();
...
//            echo '<pre>';print_r($arrEmployerWizardDetail); exit();
            ...
            $this->set('arrEmployerDomain', $arrEmployerDomain);
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->User->validates()) {
                ...
                if ($intUserCreated) {
                    ...
                    $message = "User account has been already created.";
                    $class = "alert alert-danger";
                    $icon = "icon-alert-error.png";
                    $this->set("icon", $icon);
                    ...
                    if ($this->User->save($arrPostedData)) {
                        ...
                        $arrEmployerDetailPostedData['employer_uname'] = $arrPostedData['username'];
                        $arrEmployerDetailPostedData['employer_email'] = $arrPostedData['email'];
                        ...
                        $this->Session->setFlash('You are already registered with us, please check your mail for further steps', 'default', array('class' => 'success'), 'Message');

                        $message = "You are already registered with us, please check your mail for further steps.";
                        $class = "alert alert-success";
                        $icon = "icon-alert-success.png";
                        $this->set("icon", $icon);
                        $this->set("class", $class);
                        $this->set("message", $message);
                    }
...
//            $this->layout = "employers";
        ...
        $this->loadModel('Wizemployer');
        $arrEmployerWizardDetail = $this->Wizemployer->find('all', array('conditions' => array('employer_id' => $arrLoggedUser['id'])));
//        echo '<pre>';print_r($arrEmployerWizardDetail);
        $this->loadModel('Portal');
        $arrEmployerPortalDetail = $this->Portal->find('all', array('conditions' => array('career_portal_provider_id' => $arrLoggedUser['id'])));
        
        $this->loadModel('Employerdomain');
        $arrEmployerDomainDetail = $this->Employerdomain->find('first', array('conditions' => array('employer_id' => $arrLoggedUser['id'])));
        $domain_order_id = $arrEmployerDomainDetail['Employerdomain']['order_id'];

        $this->loadModel('PortalDomain');
        $arrEmployerDomainDetail = $this->PortalDomain->find('first', array('conditions' => array('career_portal_order_id' => $domain_order_id)));
        
//        echo '<pre>';print_r($arrEmployerDomainDetail);
        $this->set("arrEmployerDetail", $arrEmployerDetail);
        $this->set("arrEmployerWizardDetail", $arrEmployerWizardDetail);
        $this->set("arrEmployerPortalDetail", $arrEmployerPortalDetail);
        $this->set("arrEmployerDomainDetail", $arrEmployerDomainDetail);
        $this->set('arrLoggedUser', $arrLoggedUser);
    }
    ...
    public function wizard_setup_theme() {
        ...
        $this->set("arrEmployerDetail", $arrEmployerWizardDetail);
        ...
        //New added code for selected theme highlighting
        $this->loadModel('PortalTheme');
        $arrTheme = $this->PortalTheme->find('first', array('conditions' => array('PortalTheme.career_portal_id =' => $intPortalDetails[0]['Portal']['career_portal_id'])));

        $this->loadModel('Wiztheme');
        $arrThemeDetail = $this->Wiztheme->find('first', array('conditions' => array('Wiztheme.theme_id' => $arrTheme["PortalTheme"]["career_portal_theme_id"])));

        $this->loadModel('Portal');
        $arrEmployerPortalDetail = $this->Portal->find('all', array('conditions' => array('career_portal_provider_id' => $arrLoggedUserDetails['id'])));
...
//        echo '<pre>';print_r($arrEmployerWizardDetail);
                ...
                if ($this->Portal->validates()) {
                    ...
                    if ($intPortalExists) {
                        ...
                        if ($boolPortalCreated) {
                            ...
                            $this->loadModel('User');
                            $boolUpdated = $this->User->updateAll(
                                    array('portal_id' => "'" . $intCreatedPortalId . "'"), array('id' => $arrLoggedUserDetails['id'])
                            );
    ...
    // wizard setup
    public function wizard_setup_publish() {
//         Configure::write('debug', 2);
        ...
        $this->loadModel('Employerdomain');
        $arrEmployerDomainDetail = $this->Employerdomain->find('first', array('conditions' => array('employer_id' => $arrLoggedUserDetails['id'])));
        $domain_order_id = $arrEmployerDomainDetail['Employerdomain']['order_id'];

        $this->loadModel('PortalDomain');
        $arrEmployerPortalDetail = $this->PortalDomain->find('first', array('conditions' => array('career_portal_order_id' => $domain_order_id)));
        
        $this->loadModel('Portal');
        $arrPortalDetails = $this->Portal->find('all', array('conditions' => array('career_portal_provider_id' => $arrLoggedUserDetails['id'])));

        $this->loadModel('Portal');
        $arrEmployerPortalName = $this->Portal->find('first', array('conditions' => array('career_portal_provider_id' => $arrEmployerPortalDetail['PortalDomain']['career_portal_domain_createdby'])));
       
        $datestr = $arrEmployerPortalDetail['PortalDomain']['career_portal_purchased_date']; //Your date
        ...
        $this->set("arrEmployerDetail", $arrEmployerDetail);
        $this->set("arrEmployerWizardDetail", $arrEmployerWizardDetail);
        $this->set("arrEmployerPortalName", $arrEmployerPortalName);
        $this->set("arrEmployerPortalDetail", $arrEmployerPortalDetail);
        $this->set("arrPortalDetails", $arrPortalDetails);
        $this->set("hourdiff", $diff);
        $this->set("days", $days);
        $this->set("hours", $hours);
    }

    public function update_wizard_publish_status() {
        $arrLoggedUserDetails = $this->Auth->user();
        $status = $_POST['status'];
        $this->loadModel('Portal');
        $this->loadModel('Employer');
        $arrEmployerPortalDetail = $this->Portal->find('first', array('conditions' => array('career_portal_id =' => $arrLoggedUserDetails['portal_id'])));
        if (count($arrEmployerPortalDetail) > 0) {
            $boolUpdated = $this->Portal->updateAll(
                    array('career_portal_published' => $status), array('career_portal_id =' => $arrLoggedUserDetails['portal_id'])
            );
            $emplyersUpdated = $this->Employer->updateAll(
                    array('is_wizard' => $status), array('employer_id =' => $arrLoggedUserDetails['id'])
            );
        }
        if ($boolUpdated) {
            $arrResponse = array();
            if ($status == '1') {
                $arrResponse['type'] = "publish";
            } else {
                $arrResponse['type'] = "unpublish";
            }
            $arrResponse['status'] = "success";
            $arrResponse['message'] = "Wizard setup publish successfully";
    ...
    public function resizeImage($source_image, $target_image, $target_width, $target_height) {
        ...
        imagecopyresampled($resized_image, $original_image, 0, 0, 0, 0, $target_width, $target_height, $source_width, $source_height);
...
//        echo $themeName.'-'.$themeColor; exit();
        ...
        $this->loadModel('Wizemployer');
        $arrEmployerWizardDetail = $this->Wizemployer->find('all', array('conditions' => array('employer_id' => $arrLoggedUserDetails['id'])));
        $this->set("arrEmployerWizDetail", $arrEmployerWizardDetail);
        $this->loadModel('Wiztheme');
        $arrThemeDetail = $this->Wiztheme->find('first', array('conditions' => array('theme_name' => $themeName, 'theme_color' => $themeColor)));

        $this->loadModel('PortalTheme');
        $arrPortalDetail = $this->PortalTheme->find('all', array('conditions' => array('career_portal_id' => $intPortalId)));
        ...
        if (count($arrPortalDetail) > 0) {
            ...
            /** 18-08-2017 Add Contact us,About us and pages **/
            // create and set default forms for portal
//            $compPortalForm = $this->Components->load('PortalFormsCreator');
//            $arrCreatedPortalForm = $compPortalForm->fnCreateDefaultPortalForms($intPortalId);
            //exit;
            // create default pages and menus for portal
//            $compPortalPages = $this->Components->load('PortalPages');
//            $arrPortalPages = $compPortalPages->fnCreateDefaultPortalPagesMenus($intPortalId);

            // create and set default theme for portal
//            $compPortalTheme = $this->Components->load('PortalTheme');
//            $arrAllocatedDefaultTheme = $compPortalTheme->fnSetDefaultTheme($intPortalId);
...
//            $arrEmployerSetup['categoryname'] = $this->request->data['Portal']['career_portal_name'];
//            $arrEmployerSetup['username'] = $arrLoggedUserDetails['email'];
//            $arrEmployerSetup['portalid'] = $intPortalId;
//            $arrLmsEmployerSetupOperation = $compLmsBridge->fnSetEmployerInMoodle($arrEmployerSetup);
            /** 18-08-2017 Add Contact us,About us and pages **/
        } else {
            ...
            $arrThemeData = array();
            $arrThemeData['career_portal_id'] = $intPortalId;
            $arrThemeData['career_portal_theme_id'] = $arrThemeDetail['Wiztheme']['theme_id'];
            $arrThemeData['career_portal_theme_active'] = '1';
            $arrThemeData['caree_portal_theme_allocatedby'] = $arrLoggedUserDetails['id'];
            $arrThemeData['career_portal_theme_allocation_datetime'] = date('Y-m-d h:i:s');
            $intBoolNewDomainInserted = $this->PortalTheme->save($arrThemeData);
            
           
        }
        $arrThemeDetails = $this->Wiztheme->find('first', array('conditions' => array('theme_name' => $themeName, 'theme_color' => $themeColor)));
        $this->set("arrThemeDetail", $arrThemeDetails);
        $this->loadModel('Portal');
...
//          print_r($arrMenuDetail); 
    ...
    public function viewthemepage($intPageId) {
        ...
        //To get Employer detail code starts
        $intPortalId = $arrPortalThemeDetail['Portal']['career_portal_id'];
        $this->loadModel('User');
                        ...
                        if (is_array($_FILES) && (count($_FILES) > 0)) {
                            ...
                            if ($_FILES['portal_logo']['name'] != "") {
                                ...
                                if (in_array($strFileExt, $arrImageAllowedExtension)) {
                                    ...
                                    $this->resizeImage($input_file, $output_file, '100', '40');
                                    $boolUpdated = $this->Portal->updateAll(
                                            array('Portal.career_portal_name' => "'" . $strPortalName . "'", 'Portal.career_portal_thumb_logo' => "'" . $strPortalNewThumbLogoName . "'", 'Portal.career_portal_logo' => "'" . $strPortalNewLogoName . "'"), array('Portal.career_portal_id =' => $intPortalId)
                                    );
                                    if ($boolUpdated) {
                                        $arrResponse = array();
                                        $arrResponse['status'] = "success";
            ...
            if ($this->Auth->loggedIn()) {
                ...
                if ($intPortlExists) {
                    ...
                    if ($this->request->data['Portal']['career_portal_footer_text'] == "") {
                        ...
                        $boolUpdated = $this->Portal->updateAll(
                                array('Portal.career_portal_footer_text' => "'" . $this->request->data['Portal']['career_portal_footer_text'] . "'"), array('Portal.career_portal_id =' => $intPortalId)
                        );
                        if ($boolUpdated) {
                            $arrResult = array();
                    ...
                    if ($arrPageDetail['0']) {
                        ...
                        if ($this->PortalPages->validates()) {
                            $boolUpdated = $this->PortalPages->updateAll(
                                    array('PortalPages.career_portal_page_tittle' => "'" . $this->request->data['PortalPages']['career_portal_page_tittle'] . "'", 'PortalPages.career_portal_page_content' => "'" . $this->request->data['PortalPages']['career_portal_page_content'] . "'", 'PortalPages.career_portal_page_template' => "'" . $this->request->data['PortalPages']['career_portal_page_template'] . "'"), array('PortalPages.career_portal_page_id =' => $arrPageDetail['0'])
                            );
    ...
    public function updategraph($intPortalId = "") {
        ...
        if ($intPortalExists) {
            ...
            $compMixPanel = $this->Components->load('MixPanel');
            $objPropertiesFilteredData = $compMixPanel->fnGetTrends($arrEvents, $strStartDate, $strEndDate, $strPeriod);
            ...
            $objSaleTrendsData = $compMixPanel->fnGetTrends($arrEvents, $strStartDate, $strEndDate, $strPeriod);
            ...
            if (is_array((array) $objSaleTrendsData->data->values) && (count((array) $objSaleTrendsData->data->values) <= 0)) {
             ...
             //echo '<pre>';print_r($arrCheckSeriesDataValues);die;
            if (isset($objPropertiesFilteredData->data->series) && isset($objPropertiesFilteredData->data->values)) {
                    ...
                    foreach ($arrSeriesDataValues as $arrSeriesDataLabel => $arrSeriesDataLabelValue) {
                        ...
                        foreach ($objPropertiesFilteredData->data->series as $arrSeries) {
                            ...
                            if ($arrSeriesDataLabel == "Monster Sale") {
                                if ($arrSeriesDataValueList[$arrSeries]) {
                                    if ($strPeriod == '30') {
                                        $strTodate = date('Y-m-t',strtotime($arrSeries));
                                        $objSaleTotal = $compMixPanel->fnSaleTotal($arrSeries, $strTodate);
                                    } else {
                                        $objSaleTotal = $compMixPanel->fnSaleTotal($arrSeries, $arrSeries);
                                    }
                ...
                $arrResponse['status'] = "success";
                if ($strPeriod == 'curr_year') {
                    $yearstr = implode(',', array_unique(explode(',', $strSeriesData)));
                    $arrResponse['xseries'] = rtrim($yearstr, ",");
                    $strSeriesDataValue = explode("~", $strTotalUsers);
                    $arrResponse['dataseries'] = "Monster Registered Users:" . $strSeriesDataValue[0] . "~Monster Sale:" . $strSeriesDataValue[1] . "";
//                    $arrResponse['dataseries'] = rtrim($strSeriesDataValue, "~");
    ...
    public function availabledomain() {

        $domainname = $_POST['domain'];
        /** Test Header * */
//        $dsc_header = array("Authorization: sso-key VVJ9uY84_M2AtrqzUXj7T4kYrPZ8R5n:UQWdvGyewMmZd9wySMxnQC", "Content-Type:application/json", "Accept:application/json");
        /** Test Header * */
        /** Server Header * */
        $dsc_header = array("Authorization: sso-key 9tzdJCoZv8j_N2kSLgmLeu4GAQxdxTNsHH:7q5eN2PmBtLQhjz3yCnXhH", "Content-Type:application/json", "Accept:application/json");
        /** Server Header * */
        $ch = curl_init("https://api.godaddy.com/v1/domains/suggest?query=" . $domainname . "");
...
//        Configure::write('debug', 2);
        ...
        $arradminId = $this->User->fnAdminDetails();
        $arrAdminDetails = $this->User->fnGetCompleteUserDetail($arradminId[0]['user']['id'], $arradminId[0]['user']['user_type']);
        $phone_no = "+" . $arrAdminDetails[0]['country_call_code'] . "." . $arrAdminDetails[0]['admin_contact_number'];
        ...
        $purchaseArray = array(
            ...
            "consent" => array(
                ...
                "agreedBy" => $arrAdminDetails[0]['admin_fname'] . " " . $arrAdminDetails[0]['admin_lname'],
                "agreedAt" => date('Y-m-d\Th:i:s') . "Z"
            ),
            "period" => 2,
            "contactAdmin" => array(
                "nameFirst" => $arrAdminDetails[0]['admin_fname'],
                "nameMiddle" => "",
                "nameLast" => $arrAdminDetails[0]['admin_lname'],
                "organization" => "",
                "jobTitle" => "",
                "email" => $arradminId[0]['user']['email'],
                "phone" => $phone_no,
                "fax" => $phone_no,
                "addressMailing" => array(
                    "address1" => $arrAdminDetails[0]['admin_address'],
                    "address2" => $arrAdminDetails[0]['admin_address1'],
                    "city" => trim($arrAdminDetails[0]['city_name'], " \r."),
                    "state" => trim($arrAdminDetails[0]['state_name'], " \r."),
                    "postalCode" => $arrAdminDetails[0]['admin_pin'],
                    "country" => $arrAdminDetails[0]['country_code']
                ),
            ),
        ...
        /** Test Header * */
//        $dsc_header = array("Authorization: sso-key VVBRynBW_GRx8QP1i7MEaa4E7zST3j2:GRxBUdE3PMoSHZ8Kaqs9oJ", "Content-Type:application/json", "Accept:application/json", "X-Shopper-Id:941015");
        /** Test Header * */
        /** Server Header * */
        ...
        $arrLoggedUserDetails = $this->Auth->user(); 
        $this->loadModel('Portal');
        $arrPortalDetail = $this->Portal->find('all', array('conditions' => array('career_portal_provider_id' => $arrLoggedUserDetails['id'])));
        
        if (isset($purchased_data->orderId)) {
            $arrEmployerDetailPostedData = array();
            $this->loadModel('PortalDomain');
            $arrEmployerDetailPostedData['career_portal_domain_name'] = $_POST['domain'];
            $arrEmployerDetailPostedData['career_portal_order_id'] = $purchased_data->orderId;
            $arrEmployerDetailPostedData['career_portal_domain_amount'] = $purchased_data->total;
            $arrEmployerDetailPostedData['career_portal_id'] = $arrPortalDetail[0]['Portal']['career_portal_id'];
            $arrEmployerDetailPostedData['career_portal_domain_createdby'] = $arrLoggedUserDetails['id'];
//            $arrEmployerDetailPostedData['career_portal_domain_server'] = Configure::read('nameServers');
            $arrEmployerDetailPostedData['career_portal_purchased_date'] = date('Y-m-d 00:00:00');
            $arrEmployerDetailPostedData['career_portal_domain_payment_status'] = "1";
            $arrEmployerDetailPostedData['career_portal_domain_status'] = "0";
            $intBoolNewPortalDomainInserted = $this->PortalDomain->save($arrEmployerDetailPostedData);

            $this->loadModel('Employerdomain');
            $arrEmployerDetailPostedData['domain_name'] = $_POST['domain'];
            $arrEmployerDetailPostedData['order_id'] = $purchased_data->orderId;
            $arrEmployerDetailPostedData['career_portal_id'] = $arrPortalDetail[0]['Portal']['career_portal_id'];
            $arrEmployerDetailPostedData['employer_id'] = $arrAdminDetails[0]['id'];
            $arrEmployerDetailPostedData['purchased_date'] = date('Y-m-d h:i:s');
            ...
            $arrEmployerDomainDetail['employer_id'] = $arrAdminDetails[0]['id'];
        ...
        if ($intBoolNewEmployerInserted) {
            $compMessage = $this->Components->load('Message');
            $strForMessage = "<div class='alert alert-success'><img alt='image description' src='http://www.rothrsolutions.com/images/icon-alert-success.png'><a aria-label='close' data-dismiss='alert' class='close' href='#'>×</a>" . $_POST['domain'] . " has been set for you, your domain will be activated in 48-72 hrs</div>";
            ...
            $compMessage = $this->Components->load('Message');
            $strForMessage = "<div class='alert alert-danger'><img alt='image description' src='http://www.rothrsolutions.com/images/icon-alert-error.png'><a aria-label='close' data-dismiss='alert' class='close' href='#'>×</a> " . $_POST['domain'] . " '" . $purchased_data->message . "'</div>";
    ...
    public function getCountryCode($country_name) {
        //Configure::write('debug', 2);
        $dsc_header = array("Content-Type:application/json", "Accept:application/json");
        $ch = curl_init("https://restcountries.eu/rest/v2/alpha/" . $country_name);
        if ($ch == FALSE) {
            echo "Connecting to createsend failed\n";
        }
        curl_setopt($ch, CURLOPT_HTTPHEADER, $dsc_header);
        ...
        $country_code = "+" . $result->callingCodes[0] . ".";
        return $country_code;
    }
    ...
    public function addportal() {
        ...
        if ($_POST['portalname'] != '') {
            $arrPostedData = array();
            $arrPostedData['career_portal_name'] = addslashes(trim(strtolower($this->request->data['portalname'])));
            $arrPostedData['career_portal_provider_id'] = $arrLoggedUserDetails['id'];
            $arrPostedData['career_portal_created_by'] = $arrLoggedUserDetails['id'];
            $arrPostedData['career_portal_created_datetime'] = date('Y-m-d h:i:s');
            
            $intBoolNewDomainInserted = $this->Portal->save($arrPostedData);
            $getInsertidPortalID = $this->Portal->getLastInsertID();
            $boolUserUpdated = $this->User->updateAll(array('portal_id' => "'$getInsertidPortalID'"), array('id' => $arrLoggedUserDetails['id']));
            
            $this->loadModel('Employerdomain');
            $boolEmployerUpdated = $this->Employerdomain->updateAll(array('career_portal_id' => "'$getInsertidPortalID'"), array('employer_id' => $arrLoggedUserDetails['id']));
            
            $this->loadModel('PortalDomain');
            $boolPortalDomainUpdated = $this->PortalDomain->updateAll(array('career_portal_id' => "'$getInsertidPortalID'"), array('career_portal_domain_createdby' => $arrLoggedUserDetails['id']));
        }

        if ($intBoolNewDomainInserted) {
            $compMessage = $this->Components->load('Message');
            $strForMessage = "<div class='alert alert-success'><img alt='image description' src='http://www.rothrsolutions.com/images/icon-alert-success.png'><a aria-label='close' data-dismiss='alert' class='close' href='#'>×</a>" . $this->request->data['portalname'] . " has been set successfully.</div>";
            $strMessage = $compMessage->fnGenerateMessageBlock($strForMessage, 'success');
            $arrResponseData['message'] = $strForMessage;
            ...
            $compMessage = $this->Components->load('Message');
            if ($this->request->data['portalname'] != '') {
                $strForMessage = "<div class='alert alert-danger'><img alt='image description' src='http://www.rothrsolutions.com/images/icon-alert-error.png'><a aria-label='close' data-dismiss='alert' class='close' href='#'>×</a> " . $this->request->data['portalname'] . " has been not set. please try again.</div>";
            } else {
                $strForMessage = "<div class='alert alert-danger'><img alt='image description' src='http://www.rothrsolutions.com/images/icon-alert-error.png'><a aria-label='close' data-dismiss='alert' class='close' href='#'>×</a> Portal name not set. please try again.</div>";
            }
...
//    public function cancelle_domain() {
//        $this->layout = NULL;
//        $this->autoRender = false;
//        $domainname = $_POST['domain'];
//        
...
//        /** Test Header * */
//        /** Server Header * */
////        $dsc_header = array("Authorization: sso-key 9tzdJCoZv8j_N2kSLgmLeu4GAQxdxTNsHH:X1H5ofQCcXChHTNHbv3ey4", "Content-Type:application/json", "Accept:application/json");
//        /** Server Header * */
//        $ch = curl_init("https://api.ote-godaddy.com/v1/domains/" . $domainname . "");
//
//        if ($ch == FALSE) {
//
//            echo "Connecting to createsend failed\n";
