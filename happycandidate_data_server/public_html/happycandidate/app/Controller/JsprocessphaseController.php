 ...
 */
class JsprocessphaseController extends AppController {
 ...
 */
	...
	public function search($strKeywordSearch)
	{
		...
		$strActionScript .= '<script type="text/javascript" src="'.Router::url('/js/jquery/jquery.tablesorter.js').'"></script>';
		$this->set('strActionScript',$strActionScript);
		$this->Session->write('strCancelUrl',Router::url(array('controller'=>'jsprocessphase','action'=>'search'),true));
		$this->loadModel('Categories');
		...
		//exit;
		$this->Paginator->settings = array(
			'conditions' => array("content_category_name LIKE"=> "%".$strKeywordSearch."%",'job_search_category'=>'1','job_process_type'=>'Phase'),
	...
	public function index() 
	{
		...
		if($this->request->is('Post') && ($this->request->data['filter_on']))
		{
			...
			$this->redirect(array('controller'=>'jsprocessphase','action'=>'search',$strProductFilterKeyword));
		}
		...
		$this->Paginator->settings = array(
			'conditions' => array('job_search_category'=>'1','job_process_type'=>'Phase'),
