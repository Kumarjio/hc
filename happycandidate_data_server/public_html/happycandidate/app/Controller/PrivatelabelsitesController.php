...
//			exit;
	...
	public function articledetail($intArticleId = "")
	{
		...
		if($intArticleId)
		{
			...
			$this->loadModel('Categories');
		        $arrCatDetail = $this->Categories->find('all',array('conditions'=>array('content_category_id'=>$arrContentDetail[0]['Content']['content_default_category'],'content_cat_for_user'=>$arrContentDetail[0]['Content']['content_for_user'])));
...
//		exit;
	...
	public function candidatedetail($intCandidateId = "")
	{
		...
		$profileDetail = $this->CandidateNew->find('first', array('conditions'=>array('hc_uid'=>$arrCandidate[0]["Candidate"]['candidate_id'])));
//		echo '<pre>';print_r($profileDetail);die;
                if(!empty($profileDetail)) {
			$arrCandidate[0]["Candidate"]["cand_phone_number"] = $profileDetail["CandidateNew"]["phone_number"];
		} else {
			$arrCandidate[0]["Candidate"]["cand_phone_number"] = "";
		}
			
		$this->set('cv_id',$arrCandidate[0]['Candidate']['cvs']['candidate_cv_id']);
...
//		exit;
	...
	{		
		...
		$arrCandidate1 = $this->Candidate->find('all',array('conditions'=>array('career_portal_id'=>$strCurrentUser['portal_id'])));
		
		$this->loadModel('CandidateNew');
		
		//$arrCandidateNew = $this->CandidateNew->find('all',array('conditions'=>array('hc_portal_id'=>$strCurrentUser['portal_id'])));
		...
		$arrCandidate1 = $this->Paginator->paginate('Candidate');
		
		$arrCandidate = array();
		
		/* foreach($arrCandidate1 as $arrCand) {
			foreach($arrCandidateNew as $arrCandNew) {
				if($arrCand["Candidate"]["candidate_id"] == $arrCandNew["CandidateNew"]["hc_uid"] && 
				$arrCand["Candidate"]["career_portal_id"] == $arrCandNew["CandidateNew"]["hc_portal_id"]){
					$arrCand["Candidate"]["cand_phone_number"] = $arrCandNew["CandidateNew"]["phone_number"];
					array_push($arrCandidate,$arrCand);
				}
		...
		foreach($arrCandidate1 as $arrCand) {
			$profileDetail = $this->CandidateNew->find('first', array('conditions'=>array('hc_uid'=>$arrCand["Candidate"]['candidate_id'])));
			//echo "<pre>"; print_r($profileDetail);
			if(!empty($profileDetail)) {
				$arrCand["Candidate"]["cand_phone_number"] = $profileDetail["CandidateNew"]["phone_number"];
				//array_push($arrCand["Candidate"]["cand_phone_number"],$profileDetail["CandidateNew"]["phone_number"]);
				array_push($arrCandidate,$arrCand);
			} else {
				$arrCand["Candidate"]["cand_phone_number"] = "";
				array_push($arrCandidate,$arrCand);
			}
		}		
		
		//echo "<pre>"; print_r($arrCandidate);
