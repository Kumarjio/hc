...
class VendorsalesController extends AppController {
                ...
                if (is_array($arrTransactionResult) && (count($arrTransactionResult) > 0)) {
                    if ($arrTransactionResult['status'] == "success") {
                        $boolContentUpdated = $this->Resourceorderdetail->updateAll(
                                array('refund_status' => "'1'", 'refund_transaction_id' => "'" . $arrTransactionResult['transid'] . "'"), array('order_detail_id =' => $arrOrderDetail[0]['Resourceorderdetail']['order_detail_id'])
                        );

                        $arrResponse['status'] = "success";
                        $arrResponse['message'] = "Refund Initiated Successfully";
                    } else {
                        $arrResponse['status'] = "fail";
                        $arrResponse['message'] = $arrTransactionResult['message'];
                    }
                } else {
                    $arrResponse['status'] = "fail";
                    $arrResponse['message'] = "There is some problem, Please try again.";
                }
            } else {
                $arrResponse['status'] = "fail";
                $arrResponse['message'] = "Please provide order Id";
            }
        } else {
            $arrResponse['status'] = "fail";
            $arrResponse['message'] = "Please provide order Id";
    ...
    public function addpayment() {
        ...
        if ($this->request->is('Post')) {
            ...
            $intAmount = $this->request->data['amount'];

            //$intVendorId = 20;
            //$intAmount = 50;

            if ($intVendorId && $intAmount) {
                $arrVendorPayments['Vendorpayments']['vendor_id'] = $intVendorId;
                $arrVendorPayments['Vendorpayments']['payedamount'] = $intAmount;
                $arrVendorPayments['Vendorpayments']['payment_made_by'] = $arrLoggedUser['id'];

                $this->Vendorpayments->create(false);
                $arrDataSaved = $this->Vendorpayments->save($arrVendorPayments);
                if (is_array($arrDataSaved) && (count($arrDataSaved) > 0)) {
                    $arrResponse['status'] = "success";
                    $arrResponse['message'] = $strMessage = $compMessage->fnGenerateMessageBlock("Payment registered successfully", 'success');

                    $arrResponse['messagenew'] = "Payment registered successfully";
                } else {
                    $arrResponse['status'] = "fail";
                    $arrResponse['message'] = $strMessage = $compMessage->fnGenerateMessageBlock("Something went wrong, Please try again.", 'error');
                }
            } else {
                $arrResponse['status'] = "fail";
                $arrResponse['message'] = $strMessage = $compMessage->fnGenerateMessageBlock("Something is missing, Please try again.", 'error');
            }
        } else {
            $arrResponse['status'] = "fail";
            $arrResponse['message'] = $strMessage = $compMessage->fnGenerateMessageBlock("Something is not correct, Please try again.", 'error');
        }
    ...
    public function salespayment($intVendorId = "", $strStartD = "", $strEndD = "") {
        //Configure::write('debug','2');
        $arrLoggedUser = $this->Auth->user();
        $this->loadModel('Vendorpayments');
        $this->loadModel('Resourceorderdetail');
        ...
        $arrConditions = array();
        if ($intVendorId) {
            $arrConditions['Vendorpayments.vendor_id'] = $intVendorId;
        }

        $strStartDate = base64_decode($strStartD);
        $strEndDate = base64_decode($strEndD);
        if ($strStartDate) {
            $arrConditions['Vendorpayments.payment_reg_date >='] = $strStartDate;
        }

        if ($strEndDate) {
            $arrConditions['Vendorpayments.payment_reg_date <='] = $strEndDate;
        }
        ...
        if (is_array($arrConditions) && (count($arrConditions) > 0)) {
            $arrProductList = $this->Vendorpayments->find('all', array(
                'conditions' => $arrConditions,
                'order' => array('vendor_payment_id' => 'desc')
            ));
        } else {
            $arrProductList = $this->Vendorpayments->find('all', array(
                'order' => array('vendor_payment_id' => 'desc')
        ...
        $this->set('arrProductList', $arrProductList);
    }

    public function salesexport($strStartD = "", $strEndD = "") {
        $arrLoggedUser = $this->Auth->user();
        $this->layout = NULL;
        $this->autoRender = false;
        $this->loadModel('Resourceorderdetail');
        ...
        if ($strStartD) {
            ...
            $strStartDate = $strStartD . " 00:00:00";
        }

        if ($strEndD) {

            $strEndDate = $strEndD . " 23:59:59";
        }
        //$strStartDate = base64_decode($strStartD);
        //$strEndDate = base64_decode($strEndD);
        /* if($this->request->is('Post'))
          {
          if($this->request->data['from_date_hid'])
          {
        ...
        if (is_array($arrNewOrders) && count($arrNewOrders) > 0) {
            ...
            $this->loadModel('Resources');
            $strFileName = "daily_sales_" . time() . ".csv";
            $strNewFh = fopen(WWW_ROOT . 'adminsales/' . $strFileName, "w");
            $intFrCnt = 0;
            $strWriteDataHeader = "Order Id,Title,Payment Status,Vendor Type, Owner, Cost, Vendor Cost, Owner Cost, Hc Cost" . ",\n";
            fputs($strNewFh, $strWriteDataHeader);
            foreach ($arrNewOrders as $arrOrder) {
                ...
                //print_r($arrOrderDetail);
                //print("<pre>");
                //print_r($arrServiceDetail);
                //print("<pre>");
                //print_r($arrCustomerDetail);
                //print("<pre>");
                //print_r($arrSubVendorsFor);
                //echo "---".$intFrCnt;
                //print("<pre>");
                //print_r($arrNewOrders[$intFrCnt]['vendorsuser']);
                ...
                if ($arrNewOrders[$intFrCnt]['Resourceorderdetail']['refund_status']) {
                    $strPaymentStatus = "Refunded";
                } else {
                    $strPaymentStatus = ucfirst($arrOrder['Resourceorderdetail']['payment_status']);
                }
                ...
                if ($arrNewOrders[$intFrCnt]['Resourceorderdetail']['refund_status']) {
                    ...
                    $strVendorCost = $arrNewOrders[$intFrCnt]['Resourceorderdetail']['vendor_cost'];
                }

                if ($arrNewOrders[$intFrCnt]['Resourceorderdetail']['refund_status']) {
                    $strOwnerCost = "0.00";
                } else {
                    $strOwnerCost = $arrNewOrders[$intFrCnt]['Resourceorderdetail']['portal_owner_cost'];
                }

                if ($arrNewOrders[$intFrCnt]['Resourceorderdetail']['refund_status']) {
                    $strHCostCost = "0.00";
                } else {
                    $strHCostCost = $arrNewOrders[$intFrCnt]['Resourceorderdetail']['hc_profit_cost'];
                }
                ...
                $arrNewOrders[$intFrCnt]['vendorsuser'] = $arrSubVendorsFor[0];
                //$strWriteData = "";
                $strWriteData = $arrNewOrders[$intFrCnt]['mainorder']['Resourceorder']['order_name'] . "," . $arrNewOrders[$intFrCnt]['service']['Resources']['product_name'] . "," . $strPaymentStatus . "," . $arrNewOrders[$intFrCnt]['Resourceorderdetail']['vendor_type'] . "," . $strVendorName . "," . $arrNewOrders[$intFrCnt]['Resourceorderdetail']['product_unit_cost'] . "," . $strVendorCost . "," . $strOwnerCost . "," . $strHCostCost . ",\n";
    ...
    public function vendorsalesexport($strStartD = "", $strEndD = "") {
        $arrLoggedUser = $this->Auth->user();
        $this->layout = NULL;
        $this->autoRender = false;
        $this->loadModel('Resourceorderdetail');
        ...
        $arrLoggedUser = $this->Auth->user();
        $this->loadModel('Vendors');
        if ($strEndDate && $strEndDate) {
            $arrProductContentList = $this->Vendors->find('all', array(
                'fields' => array('Vendors.*', 'SUM(Resourceorderdetail.vendor_cost) AS amount'),
                'joins' => array(
                    array(
                        'table' => 'resource_order_detail',
                        'alias' => 'Resourceorderdetail',
                        'type' => 'inner',
                        'conditions' => array('Resourceorderdetail.vendor_id=Vendors.vendor_id')
                    ),
                    array(
                        'table' => 'resource_order',
                        'alias' => 'Resourceorder',
                ...
                'conditions' => array('Resourceorderdetail.vendor_type' => 'vendor', 'Resourceorderdetail.refund_status' => '0', 'Resourceorderdetail.payment_status' => 'captured', 'order_detail_creation_date_time >=' => $strStartDate, 'order_detail_creation_date_time <=' => $strEndDate),
                'group' => array('Vendors.vendor_id')
            ));
        } else {
            $arrProductContentList = $this->Vendors->find('all', array(
                ...
                'conditions' => array('Resourceorderdetail.vendor_type' => 'vendor', 'Resourceorderdetail.refund_status' => '0', 'Resourceorderdetail.payment_status' => 'captured'),
                'group' => array('Vendors.vendor_id')
            ));
        }
        ...
        if (is_array($arrProductContentList) && (count($arrProductContentList) > 0)) {
            $this->loadModel('Vendorpayments');
            $intFrCnt = 0;
            $strFileName = "daily_sales_vendor_" . time() . ".csv";
            $strNewFh = fopen(WWW_ROOT . 'adminvendorsales/' . $strFileName, "w");
            $intFrCnt = 0;
            $strWriteDataHeader = "Name,Email,Owed to Vendor,Total Payments" . ",\n";
            fputs($strNewFh, $strWriteDataHeader);
            foreach ($arrProductContentList as $arrVendor) {


                $intVendorId = $arrVendor['Vendors']['vendor_id'];

                if ($strEndDate && $strEndDate) {
                    $arrSaleAmountCount = $this->Vendorpayments->find('all', array(
                        'fields' => array('SUM(payedamount) as payedamount'),
                        'conditions' => array('Vendorpayments.vendor_id' => $intVendorId, 'Vendorpayments.payment_reg_date >=' => $strEndDate, 'Vendorpayments.payment_reg_date <=' => $strEndDate)
                            )
                    );
                } else {
                    $arrSaleAmountCount = $this->Vendorpayments->find('all', array(
                        'fields' => array('SUM(payedamount) as payedamount'),
                        'conditions' => array('Vendorpayments.vendor_id' => $intVendorId)
                            )
                    );
                }
                $arrProductContentList[$intFrCnt]['Vendorpayments']['payedamount'] = $arrSaleAmountCount[0][0]['payedamount'];

                if ($arrContent['Vendorpayments']['payedamount']) {
                    $strPaymentaMount = $arrContent['Vendorpayments']['payedamount'];
                } else {
                ...
                $strWriteData = stripslashes($arrVendor['Vendors']['vendor_first_name']) . " " . stripslashes($arrVendor['Vendors']['vendor_last_name']) . "," . $arrVendor['Vendors']['vendor_email'] . "," . $arrVendor['0']['amount'] . "," . $strPaymentaMount . ",\n";
            ...
            $arrResponse['status'] = "success";
            $arrResponse['file'] = $strFileName;
            $arrResponse['filepath'] = "adminvendorsales";
        } else {
            $arrResponse['status'] = "fail";
            $arrResponse['message'] = "There are no sales in the system.";
        }
    ...
    public function sales($intVendorId = "", $strStartD = "", $strEndD = "") {
        //Configure::write('debug','2');
        $arrLoggedUser = $this->Auth->user();
        $this->loadModel('Resourceorderdetail');
        $this->loadModel('Vendors');
    ...
    public function add() {
        ...
        if ($this->request->is('Post')) {
            ...
            if ($intEditModeId) {
                ...
                if ($intContentExists) {
                    ...
                    $arrResourcesDet = $this->Resources->find('all', array('conditions' => array('productd_id' => $arrContentData['Vendorservice']['service_id'], "product_type" => "Course")));
        ...
        $this->loadModel('Vendors');
        $arrVendorDetails = $this->Vendors->find('all');
        $arrVendorServiceDetail['Vendors'] = $arrVendorDetails;
        $this->loadModel('Resources');
        $arrResourcesDetails = $this->Resources->find('all');
