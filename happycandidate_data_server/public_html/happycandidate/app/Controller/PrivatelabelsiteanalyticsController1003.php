...
//print_r($strElementHtml);die;
				...
				{
					//Configure::write('debug','2');
					$arrResponse['chartTitle'] = "Monery that Owner Have Made OverTime";

					$this->loadModel('User');

					$arrOverTimeMoneyList = $this->User->fnGetOverTimeMonery($intPortalId);

				//	echo '<pre>',print_r($arrJobSeekerPurchaseOrderList);die;
					$arrResponse['graphseariesvalue'] = count($arrOverTimeMoneyList);

					$view = new View($this, false);

					$view->set('arrOverTimeMoneyList', $arrOverTimeMoneyList);

					$strElementHtml = $view->element('overtimemoneylist');
//print_r($strElementHtml);die;
					if($strElementHtml)
