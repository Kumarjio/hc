 ...
 * @version		SVN: $Header: https://moodle2-skillsoft-activity.googlecode.com/svn/tags/2015031901/skillsoft/olsalib.php 166 2015-03-23 16:36:56Z martinholden1972@googlemail.com $
...
$CFG->skillsoft_clearwsdlcache = 0;
$CFG->skillsoft_olsaendpoint = 'http://martinsburg.skillwsa.com/olsa/services/Olsa';
$CFG->skillsoft_olsacustomerid = 'spmartinsbur';
$CFG->skillsoft_olsasharedsecret = 'wLtO3oAMjKeQHbC';
$CFG->cachedir = '.';
$CFG->proxyhost = '';
$CFG->proxyport = '';
$CFG->proxytype = '';
...
function AI_GetXmlAssetMetaData($assetid) {
	...
	if (!isolsaconfigurationset()) {
		$response = new olsaresponse(false,'Can not retrieve Skillsoft OLSA Settings: please check the configuration settings.',NULL);
	} else {
	...
	"assetId" => $assetid,
			...
			if (is_soap_fault($result)) {
				if (stripos($result->getmessage(),'security token could not be authenticated or authorized')) {
					...
					$response = new olsaresponse(false,'The OLSA Credentials are incorrect: please check the module configuration settings.',NULL);
				} elseif (stripos($result->getmessage(), 'does not exist.')){
					//Asset ID is invalid
					//print_error('olsassoapinvalidassetid','skillsoft','',$id);
					$response = new olsaresponse(false,'The Asset ID specified does not exist. Asset ID=' . $assetid,NULL);
				} else {
					//General SOAP Fault
					//print_error('olsassoapfault','skillsoft','',$result->getmessage());
					$response = new olsaresponse(false,'SOAP Fault During OLSA Call. Faultstring= ' . $result->getmessage(),NULL);
				}
			} else {
		...
		$options = array(
			...
			if (is_soap_fault($result)) {
				...
				if (!stripos($result->getmessage(),'security token could not be authenticated or authorized') == false) {
					...
					//User ID is invalid
					$response = new olsaresponse(false,'The USer ID specified does not exist. User ID=' . $userid,NULL);
		...
		$options = array(
			...
			if (is_soap_fault($result)) {
				...
				if (!stripos($result->getmessage(),'security token could not be authenticated or authorized') == false) {
					...
					$response = new olsaresponse(false,'No Results Available',NULL);
				} else {
					//General SOAP Fault
					$response = new olsaresponse(false,'SOAP Fault During OLSA Call. Faultstring= ' . $result->getmessage(),NULL);
				}
		...
		$options = array(
			...
			if (is_soap_fault($result)) {
				...
				if (!stripos($result->getmessage(),'security token could not be authenticated or authorized') == false) {
					...
					$response = new olsaresponse(false,'The report is not yet ready.',NULL);
				} elseif (!stripos($result->detail->exceptionName, 'ReportDoesNotExistFault') == false){
					//Report not ready yet
					$response = new olsaresponse(false,'The report handle specified does not exist. Handle= '. $handle,NULL);
				} else {
					//General SOAP Fault
					//print_error('olsassoapfault','skillsoft','',$result->getmessage());
					$response = new olsaresponse(false,'SOAP Fault During OLSA Call. Faultstring= ' . $result->getmessage(),NULL);
...
/**
 * Assigns catalog items to an individual user
 *
 * @param string $catalogPath the id of the courseware you want to assign
 * @param string $username the username of the student you want to assign the courseware to
 * @return olsasoapresponse olsasoapresponse->result is a NULL object
 */
function AS_SetCatalogAssignmentByUser($catalogPath = '', $username = '') {
	global $CFG;
	...
	if (!isolsaconfigurationset()) {
		...
		$options = array(
			...
			//Create the Request
			$SetCollectionAssignmentByUserRequest =  array(
				"customerId" => $customerId,
			...
			//Call the WebService and stored result in $result
			$result=$client->__soapCall('AS_SetCatalogAssignmentByUser',array('parameters'=>$SetCollectionAssignmentByUserRequest));
