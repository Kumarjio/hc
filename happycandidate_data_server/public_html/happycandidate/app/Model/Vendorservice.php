<?php
	class Vendorservice extends AppModel 
	{
		public $name = 'Vendorservice';
		...
		public function fnGetPublishedResources($limit=null)
		{
			if($limit>0)
			{
				$condition = "limit ".$limit;
                        ...
                        $strProductListQuery = "SELECT Resources.*,Vendorservice.* FROM products AS Resources,vendor_service AS Vendorservice WHERE Resources.product_publish_status = '1' AND Resources.productd_id = Vendorservice.service_id AND Vendorservice.status = 'Active' $condition"; 
                        
			$arrProductContentArray = $this->query($strProductListQuery);
			return $arrProductContentArray;
		}
							 
		public function fnGetPublishedResourcesList($limit=null,$catId="")
		{
			if($limit>0)
			{
				$condition = "limit ".$limit;
                        ...
                        if($catId !=''){
                                ...
                                $strProductListQuery = "SELECT Resources.*,Vendorservice.* FROM products AS Resources INNER JOIN vendor_service AS Vendorservice ON Resources.productd_id = Vendorservice.service_id INNER JOIN content_category_assignment AS CategoryAssign ON Resources.productd_id = CategoryAssign.product_id WHERE Resources.product_publish_status = '1' AND Vendorservice.status = 'Active' AND CategoryAssign.category_id = '".$catId."' Order by productd_id DESC";
                            
                        }else{
                            $strProductListQuery = "SELECT Resources.*,Vendorservice.* FROM products AS Resources INNER JOIN vendor_service AS Vendorservice ON Resources.productd_id = Vendorservice.service_id WHERE Resources.product_publish_status = '1' AND Vendorservice.status = 'Active' $condition Order by productd_id DESC";
                        }
...
//                public function fnGetPublishedResourcesList($limit=null,$productType="")
//		{
//			if($limit>0)
//			{
//				$condition = "limit ".$limit;
...
//                                $strProductListQuery = "SELECT Resources.*,Vendorservice.* FROM products AS Resources INNER JOIN vendor_service AS Vendorservice ON Resources.productd_id = Vendorservice.service_id WHERE Resources.product_publish_status = '1' AND Vendorservice.status = 'Active' AND product_type = '".$productType."' OR product_type = 'SkillSoftcourse' Order by productd_id DESC";
////                                $strProductListQuery = "SELECT Resources.*,Vendorservice.* FROM products AS Resources,vendor_service AS Vendorservice WHERE Resources.productd_id = Vendorservice.service_id AND Resources.product_publish_status = '1' AND Vendorservice.status = 'Active' AND product_type = '".$productType."' OR product_type = 'SkillSoftcourse' $condition GROUP BY productd_id Order by productd_id DESC";
//                            }else{
//                                $strProductListQuery = "SELECT Resources.*,Vendorservice.* FROM products AS Resources INNER JOIN vendor_service AS Vendorservice ON Resources.productd_id = Vendorservice.service_id WHERE Resources.product_publish_status = '1' AND Vendorservice.status = 'Active' AND product_type = '".$productType."' $condition Order by productd_id DESC";
////                                $strProductListQuery = "SELECT Resources.*,Vendorservice.* FROM products AS Resources,vendor_service AS Vendorservice WHERE Resources.product_publish_status = '1' AND Resources.productd_id = Vendorservice.service_id AND Vendorservice.status = 'Active' AND product_type = '".$productType."' $condition GROUP BY productd_id Order by productd_id DESC";
//                            }
//                            
//                        }else{
//                            $strProductListQuery = "SELECT Resources.*,Vendorservice.* FROM products AS Resources INNER JOIN vendor_service AS Vendorservice ON Resources.productd_id = Vendorservice.service_id WHERE Resources.product_publish_status = '1' AND Vendorservice.status = 'Active' $condition Order by productd_id DESC";
////                           $strProductListQuery = "SELECT Resources.*,Vendorservice.* FROM products AS Resources,vendor_service AS Vendorservice WHERE Resources.product_publish_status = '1' AND Resources.productd_id = Vendorservice.service_id AND Vendorservice.status = 'Active' $condition GROUP BY productd_id Order by productd_id DESC"; 
//                        }
//                        
//			$arrProductContentArray = $this->query($strProductListQuery);
//			return $arrProductContentArray;
...
//			print_r($conditions);exit;
			$strProductListQueryStart = "SELECT vendor_service.*,products.*";
			...
			{
				$strProductListQueryWhereClause = " WHERE products.productd_id = vendor_service.service_id";
//				$strProductListQueryWhereClause = " WHERE vendor_service.vendor_id = vendors.vendor_id AND products.productd_id = vendor_service.service_id";
				...
				{
					$strProductListQueryWhereClause3 = " AND products.product_name  LIKE '%".$conditions['product_name']."%'";
					$strProductListQuery = $strProductListQuery.$strProductListQueryWhereClause3;
				}
			}
                        
                        $strProductListQuery .= " Group BY vendor_service_id";
                        
                        if($conditions['column_name'] !='' && $conditions['order'] !='')
                        {
                            $strProductListQuery .= " ORDER BY ".$conditions['column_name']." ".$conditions['order']."";
                        }else{
                            $strProductListQuery .= " ORDER BY product_name ASC";
                        }
...
//				$strProductListQueryWhereClause = " WHERE vendor_service.vendor_id = vendors.vendor_id AND products.productd_id = vendor_service.service_id";
			...
			$strProductListQuery .= " Group BY vendor_service_id DESC";
