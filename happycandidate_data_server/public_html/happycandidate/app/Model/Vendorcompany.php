...
class Vendorcompany extends AppModel {

    public $name = 'Vendorcompany';
    public $useTable = 'vendor_company_details';
    ...
    public function paginate($conditions, $fields, $order, $limit, $page = 1, $recursive = null, $extra = array()) {
        ...
        $strProductListQueryFromcClause = " FROM vendors";
        ...
        if (is_array($conditions) && (count($conditions) > 0)) {
            ...
            if ($conditions['vendor_name']) {
                ...
                $strProductListQueryWhereClause3 = " vendors.vendor_name LIKE '%" . $conditions['vendor_name'] . "%'";

                $strProductListQuery = $strProductListQuery . $strProductListQueryWhereClause3;
            }
        }

        $strProductListQuery .= " ORDER BY vendor_id ASC";
    ...
    public function fnGetVendorCompanyName() {
        $strProductListQuery = "SELECT company_name,vendors.vendor_id,vendors.parent_vendor FROM `vendors`,`vendor_company_details` WHERE vendor_company_details.vendor_id=vendors.vendor_id AND vendors.parent_vendor = '0' AND vendor_active='1' ORDER BY company_name ASC";
        $arrProductContentArray = $this->query($strProductListQuery);
        return $arrProductContentArray;
    }

    public function fnGetVendorDetails($vendor_id) {
        $strProductListQuery = "SELECT vendor_company_details.company_name,address,city,state,zip,vendors.vendor_name,vendor_first_name,vendor_last_name FROM `vendors`,`vendor_company_details` WHERE vendor_company_details.vendor_id=vendors.vendor_id AND vendors.vendor_id = '".$vendor_id."'";
        $arrProductContentArray = $this->query($strProductListQuery);
        return $arrProductContentArray;
    }
}
