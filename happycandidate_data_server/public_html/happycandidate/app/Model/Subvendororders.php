<?php
	class Subvendororders extends AppModel 
	{
		public $name = 'Subvendororders';
		public $useTable = 'sub_vendor_order_service';
		...
		public function fnGetOwnerTotal($intOrderId = "",$intPortalId = "")
		{
			if($intOrderId)
			{
				$strProductListQuery = "SELECT SUM(portal_owner_cost) FROM resource_order_detail WHERE resource_order_detail.order_id = '".$intOrderId."' AND resource_order_detail.order_from_portal = '".$intPortalId."' AND resource_order_detail.payment_status = 'captured' AND resource_order_detail.order_detail_status = 'approved'";
				//echo $strProductListQuery;exit;
				$arrProductContentArray = $this->query($strProductListQuery);
				return $arrProductContentArray;
			}
		...
		public function fnGetOrderDetail($intOrderId = "")
		{
			if($intOrderId)
			{
				$strProductListQuery = "SELECT resource_order_detail.*,products.*,vendors.* FROM resource_order_detail,products,vendors WHERE resource_order_detail.order_id = '".$intOrderId."' AND resource_order_detail.product_id = products.productd_id AND resource_order_detail.vendor_id = vendors.vendor_id";
				//echo $strProductListQuery;exit;
				$arrProductContentArray = $this->query($strProductListQuery);
				return $arrProductContentArray;
			}
