<?php
	class Content extends AppModel 
	{
		...
		public function paginate($conditions, $fields, $order, $limit, $page = 1,$recursive = null, $extra = array()) 
		{
			...
			$strProductListQueryStart = "SELECT content.content_id,content_type.content_type_name,content.content_title,content.content_status,content.created_date,content.modified_date,content.content_for_user";
			$strProductListQueryFromcClause = " FROM content inner join content_type on content.content_type= content_type.content_type_id ";
			...
			if(is_array($conditions) && (count($conditions)>0))
			{
				...
				if($conditions['content_for_user']!="")
					{
						$strProductListQueryWhereClause7 = " and content.content_for_user = '".$conditions['content_for_user']."'";
						$strProductListQuery = $strProductListQuery.$strProductListQueryWhereClause7;
					}
					
				
				if($conditions['content_title'])
				{
					$strProductListQueryWhereClause3 = " AND content.content_title LIKE '%".$conditions['content_title']."%'";
					$strProductListQuery = $strProductListQuery.$strProductListQueryWhereClause3;
				}
			}
			$strProductListQuery .= " ORDER BY content_id DESC";
		...
		{
			// method body
			$strProductListQueryStart = "SELECT content.content_id,content.content_title,content.content_status,content.created_date,content.modified_date,content.content_for_user";
			$strProductListQueryFromcClause = " FROM content";
			$strProductListQuery = $strProductListQueryStart.$strProductListQueryFromcClause;
			if(is_array($conditions) && (count($conditions)>0))
			{
				$strProductListQueryWhereClause = " WHERE";
		...
		public function fnGetWebinarContentList($intCatId,$intContentType = "",$intContForUser = "3")
		{
			...
			if($intCatId)
			{
				...
				$strContentQuery = $strContentQuery." "."AND content_status = 'published' AND content_for_user = '".$intContForUser."' ORDER BY content.content_id ASC";
				//echo 'catid - '.$strContentQuery;exit;
				$strContentAsPerCat = $this->query($strContentQuery);
				return $strContentAsPerCat;
			...
			{
				 $strContentQuery =" SELECT distinct content.* FROM content,content_category,content_category_assignment WHERE content.content_id = content_category_assignment.content_id AND content_category.content_category_id = content_category_assignment.category_id AND content_status = 'published' AND content_for_user = '".$intContForUser."' and content.content_type= 2  ORDER BY content.content_id ASC";
				//echo $strContentQuery;exit;
				$strContentAsPerCat = $this->query($strContentQuery);
				return $strContentAsPerCat;
			}
		...
		public function fnGetWebinarContentListUpcoming($intCatId,$intContentType = "",$intContForUser = "3")
		{
			$strDate = date('Y-m-d');
			if($intCatId)
			{
			...
			{
				$strContentQuery =" SELECT content.* FROM content WHERE content_status = 'published' AND content_published_date >= '".$strDate."' AND content_for_user = '".$intContForUser."' and content.content_type= 2  ORDER BY content.content_id ASC";
		...
		public function fnGetSearchWebinarContentList($intPortalId = "",$arrSearchDetails = array())
		{
			$intContentType=2;
			$intContForUser=3;
			if(isset($arrSearchDetails['webinar_name']))
				{
					$searchtext = $arrSearchDetails['webinar_name'];
				}
			if(isset($arrSearchDetails['category']))
				{
					$intCatId = $arrSearchDetails['category'];
				}
				
			if($intPortalId)
			{
				
					 $strContentQuery =" SELECT distinct content.* FROM content,content_category,content_category_assignment WHERE content.content_id = content_category_assignment.content_id AND content_category.content_category_id = content_category_assignment.category_id AND content.content_type= 2";

					if($intCatId>0)
					{
						
						$strContentQuery =" SELECT distinct content.* FROM content,content_category,content_category_assignment WHERE content.content_id = content_category_assignment.content_id AND content_category.content_category_id = content_category_assignment.category_id AND content_category.content_category_id = '".$intCatId."'";
						if($intContentType)
						{
							$strContentQuery = $strContentQuery." "."AND content.content_type = '".$intContentType."'";
						}
					...
					if($searchtext!="")
					{
						if($strContentQuery!="")
						{
							$strContentQuery = $strContentQuery." "."AND content.content_title like '%".trim($searchtext)."%'";	
						}
						else
						{
							$strContentQuery =" SELECT distinct content.* FROM content,content_category,content_category_assignment WHERE content.content_id = content_category_assignment.content_id AND content_category.content_category_id = content_category_assignment.category_id AND content_status = 'published' AND content_for_user = '".$intContForUser."' and content.content_type= 2  AND content.content_title like '%".trim($searchtext)."%'";
						}
			...
			else
			{
				...
				if($searchtext!="")
				{
					$strContentQuery = $strContentQuery." "."AND content.content_title like '%".$searchtext."%'";
				}
				echo $strContentQuery;exit;
				$strContentAsPerCat = $this->query($strContentQuery);
				return $strContentAsPerCat;
