    ...
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="tab-header">
                <h1> Published Career Portals</h1>
            </div>
            <div class="tab-search">
                <?php
                $strProductSearchUrl = Router::url(array('controller' => 'manageportal', 'action' => 'publish'), true);
                ?>
            ...
            <div class="tab-row-container resourcetab">
                <table class="tablesorter panel-heading admin-content" id="sort_list">
                    ...
                    <input type="hidden" id="contentName" value="publish" />
                    <?php
                    if (is_array($arrProductList) && (count($arrProductList) > 0)) {
                        $intContentCount = 0;
                        foreach ($arrProductList as $arrContent) {
                            $careerPortalDomainName = $arrContent['career_portal_domain']['career_portal_domain_name'];
                            $careerPortalID = $arrContent['career_portal']['career_portal_id'];
                            $intContentCount++;
                            ...
                            ?>
                            <tr id="str_<?php echo $arrContent['career_portal_domain']['career_portal_domain_id']?>">
                                ...
                                <td style="width:20%!important;"><a href="http://www.<?php echo $careerPortalDomainName; ?>" target="_blank" class="link-primary"><?php echo $careerPortalDomainName; ?></a></td>
                                <td style="width:15%!important;"><?php echo date($productdateformat, strtotime($arrContent['career_portal']['career_portal_created_datetime'])) ?></td>
                                <!--<td style="width:10%!important; height: 30px !important;"><a href="javascript:void(0);" class="link-primary">Cancel</a></td>-->
                                <td style="width:10%!important; height: 30px !important;">
                                    <a href="javascript:void(0);" onclick="cancelleddomain('<?php echo $careerPortalDomainName; ?>','<?php echo $careerPortalID;?>','<?php echo $arrContent['career_portal_domain']['career_portal_domain_id']?>');" class="link-primary">Cancel</a>
                                </td>
                            </tr>
                            <?php
                        }
                        ...
                        <tr>
                            <td colspan='7'><span style="margin-left:40%;">No Publish Career Portal</span></td>
                        </tr>
                        <tr>
                            <td colspan='7' class="altrow">&nbsp;</td>
                        </tr>
...
<script type="text/javascript">
    ...
    function cancelleddomain(domainName,portalId,domain_id) {
        var message = confirm('Are you sure want to cancelled domain?');
        if (message == true) {
            $('.cms-bgloader-mask').show();//show loader mask
            $('.cms-bgloader').show(); //show loading image 
            $("#response_msg").html('');
            $.ajax({
                url: "<?php echo Router::url(array('controller' => 'manageportal', 'action' => 'canceldomain'), true); ?>",
                type: "POST",
                dataType: 'json',
                data: 'domain=' + domainName+'&portalId='+portalId+'&domainId='+domain_id,
                success: function (Response)
                {
                    if (Response.status == 'success')
                    {
//                        location.reload();
                        $('#str_'+Response.domainId).remove();//remove row
                        $('.cms-bgloader-mask').hide();//show loader mask
                        $('.cms-bgloader').hide(); //show loading image 
                        $("#product_notification").html(Response.message);
                    } else
                    {
                        $('.cms-bgloader-mask').hide();//show loader mask
                        $('.cms-bgloader').hide(); //show loading image 
