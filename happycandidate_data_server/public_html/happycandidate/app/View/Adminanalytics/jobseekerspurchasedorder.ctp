<div class="page-content-wrapper employers-type" style="min-height:500px;">
    <div class="container-fluid">
        <div class="row">
            <div class="col-lg-12">
                <h1>Job seeker(s) <?php echo $productTypes;?> Purchased Order(s)</h1>
                <div class="tab-row-container">
                    <p>Listed are all job seeker(s) <?php echo $productTypes;?> purchased order(s) in your Career Portal</p>
                    ...
                    <div class="panel panel-default hidden-xs hidden-sm">
                        ...
                        if (is_array($arrJobSeekerPurchaseOrderList) && (count($arrJobSeekerPurchaseOrderList) > 0)) { ?>
                                    ...
                                    if (is_array($arrJobSeekerPurchaseOrderList) && (count($arrJobSeekerPurchaseOrderList) > 0)) {
                                        ...
                                        foreach ($arrJobSeekerPurchaseOrderList as $arrJobSeekerPurchaseOrder) {
                                            ...
                                            <tr <?php echo $class; ?>>
                                                ...
                                                <td>$<?php echo isset($arrJobSeekerPurchaseOrder['resource_order_detail']['total']) ? ($arrJobSeekerPurchaseOrder['resource_order_detail']['total']) : '0.00'; ?></td>
                                                <td><?php echo date($productdateformat, strtotime($arrJobSeekerPurchaseOrder['resource_order']['order_creation_date_time'])); ?></td>
                                            </tr>
                                            <?php
                                        }
                                    }
                            ...
                            <table>
                                ...
                                <tr>
                                    <td colspan='5'><span style="margin-left:20%;">No job seeker(s) <?php echo $productTypes;?> sale purchased order(s)</span></td>
                                </tr>
                            </table>
                            <?php
                        }
...
<script type="text/javascript">
	...
	var PortalId = '<?php echo $portalId;?>';
        ...
        var strProductType = '<?php echo $productType;?>';
		
	$('.cms-bgloader-mask').show();//show loader mask
	$('.cms-bgloader').show(); //show loading image
	$.ajax({ 
            type: "GET",
            url: appBaseU+"adminanalytics/purchaseOrderExport",
            data: 'StartDate='+strStartDate+"&EndDate="+strEndDate+"&portalId="+PortalId+"&productType="+strProductType,
            cache: false,
            dataType:"json",
