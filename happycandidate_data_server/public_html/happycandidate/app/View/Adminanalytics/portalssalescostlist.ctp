<div class="page-content-wrapper employers-type" style="min-height:500px;">
    <div class="container-fluid">
        <div class="row">
            <div class="col-lg-12">
                <h1>Portal Sale Cost</h1>
                <div class="tab-row-container">
                    <p>Listed are all portal sale cost in your Career Portal</p>
                    ...
                    <button name="export" id="export" type="button" class="btn btn-default btn-md export-btn" onclick="fnExportPortalDomainSaleCost()" >Export</button>
                </div>
                <div class="tab-row-container">
                    <div class="panel panel-default hidden-xs hidden-sm">
                        ...
                        <?php if (is_array($arrPortalDomainList) && (count($arrPortalDomainList) > 0)) { ?>
                            <div class="panel-body emp-dashboard-jobs emp-dashboard-pages">
                                <table>
                                    ...
                                    if (is_array($arrPortalDomainList) && (count($arrPortalDomainList) > 0)) {
                                        $intForI = 0;
                                        $class = null;
                                        foreach ($arrPortalDomainList as $arrPortalDomain) {
                                            if ($intForI++ % 2 == 0) {
                                                $class = ' class="altrow"';
                                            }
                                            ?>
                                            <tr <?php echo $class; ?>>
                                                ...
                                                <td><?php echo $arrPortalDomain['career_portal_domain']['career_portal_domain_name']; ?></td>
                                                <td><?php echo $arrPortalDomain['career_portal_domain']['career_portal_order_id']; ?></td>
                                                <td><?php echo isset($arrPortalDomain['career_portal_domain']['career_portal_domain_amount']) ? '$'.($arrPortalDomain['career_portal_domain']['career_portal_domain_amount']) : '$0.00'; ?></td>
                                                <td><div class="user-title"><?php echo $arrPortalDomain['PortalName']['pname']; ?></div></td>
                                                <td><?php if($arrPortalDomain['career_portal_domain']['career_portal_domain_status']) { echo "Active";}else{    echo 'Inactive';} ?></td>
                                                <td><?php echo date($productdateformat, strtotime($arrPortalDomain['career_portal_domain']['career_portal_purchased_date'])); ?></td>
                                            </tr>
                                            <?php
                                            $count++;
                                        }
                            ...
                            <table>
                                ...
                                <tr>
                                    <td colspan='6'><span style="margin-left:20%;">No portal sale cost</span></td>
                                </tr>
                            </table>
                            <?php
                        }
...
<script type="text/javascript">
    ...
    function fnExportPortalDomainSaleCost()
    {
        ...
        $.ajax({
            type: "GET",
            url: appBaseU + "adminanalytics/portalssalescostExport",
