...
<div class="page-content-wrapper employers-type" style="min-height:500px;">
    <div class="container-fluid">
        <div class="row">
            <div class="col-lg-12">
                <h1>Total Refunds For <?php echo stripslashes($vendorName);?> Vendor</h1>
                <div class="tab-row-container">
                    <p>Listed are total refunds for <?php echo stripslashes($vendorName);?> vendor in your Career Portal</p>
                    ...
                    <div class="panel panel-default hidden-xs hidden-sm">
                        ...
                        if (is_array($arrRefundedOrders) && (count($arrRefundedOrders) > 0)) { ?>
                            <div class="panel-body emp-dashboard-jobs emp-dashboard-pages">
                                <table>
                                    ...
                                    if (is_array($arrRefundedOrders) && (count($arrRefundedOrders) > 0)) {
                                        $intForI = 0;
                                        $class = null;
                                        foreach ($arrRefundedOrders as $RefundedOrders) {
                                            if ($intForI++ % 2 == 0) {
                                                $class = ' class="altrow"';
                                            }
                                            ?>
                                            <tr <?php echo $class; ?>>
                                                ...
                                                <td><?php echo $RefundedOrders['career_portal_candidate']['candidate_email']; ?></td>
                                                <td><div class="user-title"><?php echo $RefundedOrders['career_portal_candidate']['candidate_first_name']; ?></div></td>
                                                <td><?php echo $RefundedOrders['career_portal_candidate']['candidate_last_name']; ?></td>
                                                <td>$<?php echo $RefundedOrders['resource_order_detail']['vendor_cost']; ?></td>
                                                <td><?php echo date($productdateformat, strtotime($RefundedOrders['resource_order_detail']['order_detail_creation_date_time'])); ?></td>
                            ...
                            <table>
                                ...
                                <tr>
                                    <td colspan='5'><span style="margin-left:20%;">No total refunds for <?php echo stripslashes($vendorName);?> vendor</span></td>
                                </tr>
                            </table>
                            <?php
                        }
...
<script type="text/javascript">
	...
	$.ajax({ 
            type: "GET",
            url: appBaseU+"adminanalytics/vendorOrderRefundedExport",
            data: 'StartDate='+strStartDate+"&EndDate="+strEndDate+"&portalId="+PortalId+"&Vendor="+strVendor,
            cache: false,
            dataType:"json",
