			...
			<div class="col-md-12 resume-builder-8-awards flex-fix">
				...
				<div class="aside-steps-container bg-lightest-grey">
					...
					<ul class="resume-builder-steps-list" style="padding:0px;">
						...
						<li class="resume-builder-step-current" style="cursor:pointer;" onClick="return getMyTeaching('<?php echo $intPortalId?>','<?php echo $resumeid;?>');">
			 ...
			 --><div class="content-step">
					...
					<form id="teaching" name="teaching" method="post">
				...
				<?php
					...
					if(count($candidateawards)>0)
					{
					   ...
					   {
						   $candidate_awards_id = $arrEdu['Candidate_teaching']['candidate_awards_id'];
						   $candidate_cv_id = $arrEdu['Candidate_teaching']['candidate_cv_id'];
						   $award = $arrEdu['Candidate_teaching']['title'];
						   $organization = $arrEdu['Candidate_teaching']['organization'];
						   $description = $arrEdu['Candidate_teaching']['location'];
						   $date = $arrEdu['Candidate_teaching']['date'];
						   $intFrCnt++;
						   ?>
								<div id="awards<?php echo $intFrCnt;?>">
									<div class="form-group">
										...
										<label class="control-label" for="award-name">Name of Course:</label>
					...
					?>
					<input type="hidden" name="teaching_count" id="teaching_count" value="<?php echo $intFrCnt; ?>" />
					<div id="awardd"></div>
						...
						<div class="form-group">
							<button class="btn btn-default btn-responsive btn-lg"  type="button" onClick="return getMyCampus('<?php echo $intPortalId?>','<?php echo $resumeid;?>');">&lt;&nbsp;Prev</button>
							<button class="btn btn-primary btn-responsive btn-lg" type="button" onClick="return saveTeaching('<?php echo $intPortalId?>','0');">Next&nbsp;&gt;</button>
							<a href="#" class="link link-default" onClick="return saveTeaching('<?php echo $intPortalId?>','0');">Skip this step</a>
						</div>
					</form>
				</div>
				<div class="aside-actions-container">
					<h2>Definition</h2>
					<p>
						Subdivide this section of your CV either by field of teaching or by institutional location or by Graduate/Undergraduate.  You can also use some combination of these as appropriate to your specific experience.
					</p>
					<p>
						If you’ve taught in more than one institution make a subheading for each institution.  Then list the courses vertically down the left.  To the right of each course in parentheses give the terms and years taught.  This allows you to show the number of times you’ve taught a course without listing it repeatedly.  If you have taught a course in excess of fifteen times condense this section.
					</p>
					<div class="panel-fixed-controls">
						<a href="javascript:void(0);" onClick="return saveTeaching('<?php echo $intPortalId?>','1');">
...
<script type="text/javascript">
	...
	{
		var intEdCount = $('#teaching_count').val();
		$('#awards'+intEdCount).remove();
		intEdCount--;
		$('#teaching_count').val(intEdCount);
		if(intEdCount == 1)
		{
			$('#btnremove').hide();
		}
