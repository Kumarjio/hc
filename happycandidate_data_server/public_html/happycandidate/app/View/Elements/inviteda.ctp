			...
			<div class="col-md-12 resume-builder-8-awards flex-fix">
				...
				<div class="aside-steps-container bg-lightest-grey">
					...
					<ul class="resume-builder-steps-list" style="padding:0px;">
						...
						<li class="resume-builder-step-current" style="cursor:pointer;" onClick="return getMyInvites('<?php echo $intPortalId?>','<?php echo $resumeid;?>');">
			 ...
			 --><div class="content-step">
					...
					<form id="invited" name="invited" method="post">
				...
				<?php
					...
					if(count($candidateawards)>0)
					{
					   ...
					   {
						   $candidate_awards_id = $arrEdu['Candidate_invited']['candidate_awards_id'];
						   $candidate_cv_id = $arrEdu['Candidate_invited']['candidate_cv_id'];
						   $award = $arrEdu['Candidate_invited']['title'];
						   $organization = $arrEdu['Candidate_invited']['organization'];
						   $description = $arrEdu['Candidate_invited']['location'];
						   //$date = $arrEdu['Candidate_invited']['date'];
						   $date = $arrEdu['Candidate_invited']['year'];
						   $intFrCnt++;
						   ?>
					...
					else
					{
						...
						?>
							<div id="awards<?php echo $intFrCnt;?>">
								...
								<div class="form-group">
									<input type="hidden" name="awardsid<?php echo $intFrCnt;?>" id="awardsid<?php echo $intFrCnt;?>" value=""/>	
									<label class="control-label" for="award-name">Title:</label>
					...
					?>
					<input type="hidden" name="invited_count" id="invited_count" value="<?php echo $intFrCnt; ?>" />
					<div id="awardd"></div>
						...
						<div class="form-group">
							<button class="btn btn-default btn-responsive btn-lg"  type="button" onClick="return getMyGrants('<?php echo $intPortalId?>','<?php echo $resumeid;?>');">&lt;&nbsp;Prev</button>
							<button class="btn btn-primary btn-responsive btn-lg" type="button" onClick="return saveInvite('<?php echo $intPortalId?>','0');">Next&nbsp;&gt;</button>
							<a href="#" class="link link-default" onClick="return saveInvite('<?php echo $intPortalId?>','0');">Skip this step</a>
						</div>
					</form>
				</div>
				<div class="aside-actions-container">
					<h2>Definition</h2>
					<p>
						This section of your CV is listed if you have been invited by other campuses to provide a talk.
					</p>
					<div class="panel-fixed-controls">
						<a href="javascript:void(0);" onClick="return saveInvite('<?php echo $intPortalId?>','1');">
...
<script type="text/javascript">
	...
	{
		var intEdCount = $('#invited_count').val();
		$('#awards'+intEdCount).remove();
		intEdCount--;
		$('#invited_count').val(intEdCount);
		if(intEdCount == 1)
		{
			$('#btnremove').hide();
		}
