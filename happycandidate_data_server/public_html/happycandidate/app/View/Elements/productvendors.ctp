        ...
        if (is_array($arrVendorServiceDetail['Vendors']) && (count($arrVendorServiceDetail['Vendors']) > 0)) {
            foreach ($arrVendorServiceDetail['Vendors'] as $arrVendor) {
                ...
                <option <?php
                        ...
                        ?> value="<?php echo $arrVendor['Vendors']['vendor_id']; ?>"><?php echo $arrVendor['Vendors']['vendor_first_name'] . " " . $arrVendor['Vendors']['vendor_last_name']; ?></option>
                <?php
            }
        }
        ?>
...
<div class="form-group">
    <label class="control-label col-xs-12 col-sm-12 col-md-4" for="name">Product Name: <span class="form-required">*</span></label>
...
<script>
    function select_all() {
    $('#vendor option').prop('selected', true);
    
    var selected = $("#vendor option:selected");
        var message = "";
