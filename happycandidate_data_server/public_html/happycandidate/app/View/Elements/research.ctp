			...
			<div class="col-md-12 resume-builder-8-awards flex-fix">
				...
				<div class="aside-steps-container bg-lightest-grey">
					...
					<ul class="resume-builder-steps-list" style="padding:0px;">
						...
						<li class="resume-builder-step-current" style="cursor:pointer;" onClick="return getMyResearch('<?php echo $intPortalId?>','<?php echo $resumeid;?>');">
			 ...
			 --><div class="content-step">
					...
					<form id="research" name="research" method="post">
				...
				<?php
					...
					if(count($candidateawards)>0)
					{
					   ...
					   {
						   $candidate_awards_id = $arrEdu['Candidate_research']['candidate_awards_id'];
						   $candidate_cv_id = $arrEdu['Candidate_research']['candidate_cv_id'];
						   $award = $arrEdu['Candidate_research']['research_exp'];
						   $organization = $arrEdu['Candidate_research']['organization'];
						   $description = $arrEdu['Candidate_research']['location'];
						   $date = $arrEdu['Candidate_research']['date'];
						   $intFrCnt++;
						   ?>
								<div id="awards<?php echo $intFrCnt;?>">
									<div class="form-group">
										...
										<label class="control-label" for="award-name">Description of Research Experience (sentence form):</label>
					...
					?>
					<input type="hidden" name="research_count" id="research_count" value="<?php echo $intFrCnt; ?>" />
					<div id="awardd"></div>
						...
						<div class="form-group">
							<button class="btn btn-default btn-responsive btn-lg"  type="button" onClick="return getMyTeaching('<?php echo $intPortalId?>','<?php echo $resumeid;?>');">&lt;&nbsp;Prev</button>
							<button class="btn btn-primary btn-responsive btn-lg" type="button" onClick="return saveResearch('<?php echo $intPortalId?>','0');">Next&nbsp;&gt;</button>
							<a href="#" class="link link-default" onClick="return saveResearch('<?php echo $intPortalId?>','0');">Skip this step</a>
						</div>
					</form>
				</div>
				<div class="aside-actions-container">
					<h2>Definition</h2>
					<p>
						RA experience is listed under this section of your CV, as well as lab experience.  This is one section where slight elaboration is possible, if the research was a team effort on a complex, multi-year theme.  One detailed sentence should suffice
					</p>
					<p>
					...
					<div class="panel-fixed-controls">
						<a href="javascript:void(0);" onClick="return saveResearch('<?php echo $intPortalId?>','1');">
...
<script type="text/javascript">
	...
	{
		var intEdCount = $('#research_count').val();
		$('#awards'+intEdCount).remove();
		intEdCount--;
		$('#research_count').val(intEdCount);
		if(intEdCount == 1)
		{
			$('#btnremove').hide();
		}
	...
	function fnAddAward() 
	{
		...
		var strEduRows = '<div id="awards'+intEdCount+'"><div class="form-group"><input type="hidden" name="awardsid'+intEdCount+'" id="awardsid'+intEdCount+'" value="<?php echo $candidate_awards_id;?>"/><label class="control-label" for="award-name">Description of Research Experience (sentence form):</label><textarea class="builder-textarea" name="description" id="description" rows="4"><?php echo $award;?></textarea></div></div>';
