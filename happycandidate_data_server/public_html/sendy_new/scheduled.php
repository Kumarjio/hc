...
//        preg_match_all('/(?P<name>\w+)_purchase_link/',$html,$matches,PREG_PATTERN_ORDER);
        preg_match_all('/\[([a-zA-Z0-9!#%^&*()+=$@._\-\:|\/?<>~`"\'\s]+)_purchase_link]/',$html,$matches,PREG_PATTERN_ORDER);
        $pro_code = ($matches[0][0]);
        $servicesCode = $matches[1][0];
        $servicesName = str_replace("_"," ",$matches[1][0]);
        
        $servicesq = "SELECT productd_id,product_name FROM products WHERE product_name LIKE '%$servicesName%'";
        $pro_name = mysqli_query($conn, $servicesq);
        
        if ($pro_name) {
            while ($productsList = mysqli_fetch_array($pro_name)) {
        ...
        if ($product_id) {
            $strProductListQuery = "SELECT vendor_service_id FROM vendor_service WHERE service_id = '" . $product_id . "' LIMIT 1";
            $vendorservices = mysqli_query($conn, $strProductListQuery);

            if ($vendorservices) {
                while ($vendorService = mysqli_fetch_array($vendorservices)) {
                    $vendorServicesId = $vendorService['vendor_service_id'];
                }
            }
         }
        //Set language
        ...
        if ($r_l && mysqli_num_rows($r_l) > 0)
            while ($row = mysqli_fetch_array($r_l))
        ...
        if ((($time >= $send_date && $time < $send_date + 300) && $sent == '') || (($send_date < $time) && $sent == '') || ($send_date == '0' && $timezone == '0')) {
            ...
            if ($r && mysqli_num_rows($r) > 0) {
                ...
                while ($row = mysqli_fetch_array($r)) {
                            ...
                            //otherwise, replace custom field tag
                                ...
                                if ($r5) {
                                    while ($row2 = mysqli_fetch_array($r5))
                                        $custom_fields = $row2['custom_fields'];
                    ...
                    //Get List table data
                    if ($custom_values != '') {
                        $list_q = 'SELECT user_type,portal_id,subscribe_status FROM lists WHERE id = ' . $subscriber_list;
                        $list_r = mysqli_query($mysqli, $list_q);
                        if ($list_r) {
                            $userType = '';
                            $portalID = '';
                            $subscribeType = '';
                            while ($arr_list = mysqli_fetch_array($list_r)) {
                                
                                $line_array = '';
                                $userType = $arr_list['user_type'];
                                $portalID = $arr_list['portal_id'];
                                $subscribeType = $arr_list['subscribe_status'];
                                ...
                                if ($userType == 'owner') {
                                    if ($portalID != '' && $portalID != '0') {
                                        if ($subscribeType == 'all') {
                                            $q = 'SELECT user.email,portal_id,employer_uname,employer_user_fname,employer_user_lname,employer_company_name,employer_country,employer_state,employer_city,employer_pin,employer_contact_number,domain_name FROM user INNER JOIN employer_detail INNER JOIN employer_domain WHERE user.id = employer_detail.employer_id AND employer_detail.employer_id = employer_domain.employer_id AND user.email="' . $email . '" AND portal_id="' . $portalID . '" AND user_type=2';
                                        } else {
                                            $q = 'SELECT user.email,portal_id,employer_uname,employer_user_fname,employer_user_lname,employer_company_name,employer_country,employer_state,employer_city,employer_pin,employer_contact_number,domain_name FROM user INNER JOIN employer_detail INNER JOIN employer_domain WHERE user.id = employer_detail.employer_id AND employer_detail.employer_id = employer_domain.employer_id AND is_active ="' . $subscribeType . '" AND user.email="' . $email . '" AND portal_id="' . $portalID . '" AND user_type=2';
                                        }
                                    } else {
                                        if ($subscribeType == 'all') {
                                            $q = 'SELECT user.email,portal_id,employer_uname,employer_user_fname,employer_user_lname,employer_company_name,employer_country,employer_state,employer_city,employer_pin,employer_contact_number,domain_name FROM user INNER JOIN employer_detail INNER JOIN employer_domain WHERE user.id = employer_detail.employer_id AND employer_detail.employer_id = employer_domain.employer_id AND user.email="' . $email . '" AND user_type=2';
                                        } else {
                                            $q = 'SELECT user.email,portal_id,employer_uname,employer_user_fname,employer_user_lname,employer_company_name,employer_country,employer_state,employer_city,employer_pin,employer_contact_number,domain_name FROM user INNER JOIN employer_detail INNER JOIN employer_domain WHERE user.id = employer_detail.employer_id AND employer_detail.employer_id = employer_domain.employer_id AND is_active ="' . $subscribeType . '" AND user.email="' . $email . '" AND user_type=2';
                                        }
                                    }
                                    ...
                                    if ($subscribeType == 'all') {
                                        $q = 'SELECT vendor_email,vendor_first_name,vendor_last_name,company_name,vendor_name,zip,city,state,country,phone FROM vendors INNER JOIN vendor_company_details';
                                    } else {
                                        $q = 'SELECT vendor_email,vendor_first_name,vendor_last_name,company_name,vendor_name,zip,city,state,country,phone FROM vendors INNER JOIN vendor_company_details WHERE vendor_active ="' . $subscribeType . '"';
                                    }
                                    ...
                                    if ($r) {
                                     ...
                                     if ($portalID != '' && $portalID != '0') {
                                        if ($subscribeType == 'all') {
                                            $q = 'SELECT career_portal_candidate.career_portal_id,career_portal_candidate.candidate_first_name,career_portal_candidate.candidate_last_name,career_portal_candidate.candidate_email,career_portal_candidate.candidate_username,career_portal_domain.career_portal_domain_name FROM career_portal_candidate INNER JOIN career_portal_domain WHERE career_portal_candidate.career_portal_id = career_portal_domain.career_portal_id AND career_portal_id="' . $portalID . '" AND candidate_email="' . $email . '"';
                                        }else{
                                            $q = 'SELECT career_portal_candidate.career_portal_id,career_portal_candidate.candidate_first_name,career_portal_candidate.candidate_last_name,career_portal_candidate.candidate_email,career_portal_candidate.candidate_username,career_portal_domain.career_portal_domain_name FROM career_portal_candidate INNER JOIN career_portal_domain WHERE career_portal_candidate.career_portal_id = career_portal_domain.career_portal_id AND career_portal_id="' . $portalID . '" AND candidate_is_active ="' . $subscribeType . '" AND candidate_email="' . $email . '"';
                                        }
                                     }else{
                                        if ($subscribeType == 'all') {
                                            $q = 'SELECT career_portal_candidate.career_portal_id,career_portal_candidate.candidate_first_name,career_portal_candidate.candidate_last_name,career_portal_candidate.candidate_email,career_portal_candidate.candidate_username,career_portal_domain.career_portal_domain_name FROM career_portal_candidate INNER JOIN career_portal_domain WHERE career_portal_candidate.career_portal_id = career_portal_domain.career_portal_id AND candidate_email="' . $email . '"';
                                        }else{
                                           $q = 'SELECT career_portal_candidate.career_portal_id,career_portal_candidate.candidate_first_name,career_portal_candidate.candidate_last_name,career_portal_candidate.candidate_email,career_portal_candidate.candidate_username,career_portal_domain.career_portal_domain_name FROM career_portal_candidate INNER JOIN career_portal_domain WHERE career_portal_candidate.career_portal_id = career_portal_domain.career_portal_id AND candidate_is_active ="' . $subscribeType . '" AND candidate_email="' . $email . '"'; 
                                        } 
                                     }
                        ...
                        if ($userType == 'owner') {
                            $firstname = strip_tags($line_array[0]['employer_user_fname']);
                            $lastname = strip_tags($line_array[0]['employer_user_lname']);
                            $fullname = strip_tags($line_array[0]['employer_user_fname'] . " " . $line_array[0]['employer_user_lname']);
                            $username = strip_tags($line_array[0]['employer_uname']);
                            $email = trim($line_array[0]['email']);
                            $companyname = strip_tags($line_array[0]['employer_company_name']);
                            $country = strip_tags($line_array[0]['employer_country']);
                            $state = strip_tags($line_array[0]['employer_state']);
                            $city = strip_tags($line_array[0]['employer_city']);
                            $zipcode = strip_tags($line_array[0]['employer_pin']);
                            $phonenumber = strip_tags($line_array[0]['employer_contact_number']);
                            $portal_url = strip_tags($line_array[0]['domain_name']);
                            $forgot_password_url = $_SERVER['HTTP_HOST'] . '/happycandidate/users/forgotpassword';
                            $forgot_password_link = '<a href="' . $forgot_password_url . '" class="link-primary">Forgot Password?</a>';
                            
                            $purchase_url = $_SERVER['HTTP_HOST'] . '/happycandidate/resources/detail/'.$line_array[0]['portal_id']."/".$vendorServicesId;
                            $purchase_link = '<a href="' . $purchase_url . '" class="link-primary">'.$servicesName.' Purchase Link</a>';
                            
                        } else if ($userType == 'vendor') {
                            $firstname = strip_tags($line_array[0]['vendor_first_name']);
                            $lastname = strip_tags($line_array[0]['vendor_last_name']);
                            $fullname = strip_tags($line_array[0]['vendor_first_name'] . " " . $line_array[0]['vendor_last_name']);
                            $username = strip_tags($line_array[0]['vendor_name']);
                            $email = trim($line_array[0]['vendor_email']);
                            $companyname = strip_tags($line_array[0]['company_name']);
                            $country = strip_tags($line_array[0]['country']);
                            $state = strip_tags($line_array[0]['state']);
                            $city = strip_tags($line_array[0]['city']);
                            $zipcode = strip_tags($line_array[0]['zip']);
                            $phonenumber = strip_tags($line_array[0]['phone']);
                            $portal_url = strip_tags($line_array[0]['domain_name']);
                            $forgot_password_url = $_SERVER['HTTP_HOST'] . '/happycandidate/vendoraccount/forgotpassword';
                            $forgot_password_link = '<a href="' . $forgot_password_url . '" class="link-primary">Forgot Password?</a>';
                            
                        } else {
                            $firstname = strip_tags($line_array[0]['candidate_first_name']);
                            $lastname = strip_tags($line_array[0]['candidate_last_name']);
                            $fullname = strip_tags($line_array[0]['candidate_first_name'] . " " . $line_array[0]['candidate_last_name']);
                            $username = strip_tags($line_array[0]['employer_uname']);
                            $email = trim($line_array[0]['candidate_email']);
                            $portal_url = strip_tags($line_array[0]['career_portal_domain_name']);
                            $forgot_password_url = $_SERVER['HTTP_HOST'] . '/happycandidate/portal/forgotpassword';
                            $forgot_password_link = '<a href="' . $forgot_password_url . '" class="link-primary">Forgot Password?</a>';
                            $purchase_url = $_SERVER['HTTP_HOST'] . '/happycandidate/resources/detail/'.$line_array[0]['career_portal_id']."/".$vendorServicesId;
                            $purchase_link = '<a href="' . $purchase_url . '" class="link-primary">'.$servicesName.' Purchase Link</a>';
                        }
                        ...
                        $html_treated = str_replace($a, $b, $html_treated);
                        $plain_treated = str_replace($a, $b, $plain_treated);
                    } else {
                ...
                $headers[] = "X-Amzn-Authorization: AWS3-HTTPS " . "AWSAccessKeyId=" . $s3_key . "," . "Algorithm=HmacSHA1,Signature={$signature}";
            ...
            if ($r8) {
                while ($row = mysqli_fetch_array($r8)) {
                    $no_of_recipients = $row['recipients'];
                    if ($no_of_recipients >= $to_send) {
                        ...
                        if ($to_send_num == '' || $to_send > $to_send_num)
                            $to_send_num = $to_send;
