...
if (trim($html) == '<html><head></head><body></body></html>')
    $html = '';
...
if (isset($_POST['save-only']))
    $save_only = is_numeric($_POST['save-only']) ? $_POST['save-only'] : 0;
...
if (!file_exists("../../uploads/attachments"))
    mkdir("../../uploads/attachments", 0777);
...
if ($r)
    while ($row = mysqli_fetch_array($r))
        $allowed = array_map('trim', explode(',', $row['allowed_attachments']));
