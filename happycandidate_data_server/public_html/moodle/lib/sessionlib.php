...
abstract class session_stub implements moodle_session {
    ...
    public function __construct() {
        ...
        if (NO_MOODLE_COOKIES) {
			...
			switch($CFG->currentsessionvalue)
			{
				case"admin": $newsession = empty($_COOKIE[$CFG->currentsessiontype.$CFG->sessioncookie]);
							 ini_set('session.use_trans_sid', '0');
							 session_name($CFG->currentsessiontype);
							 break;
							 
				default:	 $newsession = empty($_COOKIE[$CFG->currentsessiontype.$CFG->currentsessionvalue.$CFG->sessioncookie]);
							 ini_set('session.use_trans_sid', '0');
							 session_name($CFG->currentsessiontype.$CFG->currentsessionvalue);
							 $strSessionId = $_COOKIE[$CFG->currentsessiontype.$CFG->currentsessionvalue];
