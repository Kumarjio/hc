...
class moodle_url {
    ...
    public function __construct($url, array $params = null) {
        ...
        if ($url instanceof moodle_url) {
			...
			if(is_array($arrUrlParts) && (count($arrUrlParts)>0))
			{
				if(isset($arrUrlParts[1]))
				{
					if(stripos($arrUrlParts[1],$CFG->currentsessionvar.'='.$CFG->currentsessionvalue) === false)
					{
						$strUrl = $strUrl."&".$CFG->currentsessionvar.'='.$CFG->currentsessionvalue;
					}
				}
				else
				{
