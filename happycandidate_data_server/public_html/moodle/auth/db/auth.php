...
class auth_plugin_db extends auth_plugin_base {
    ...
    function user_login($username, $password) {
        ...
        if ($this->is_internal()) {
			...
			else if($this->config->passtype === 'cake')
			{
				$extpassword = md5($CFG->cakesaltvalue.$extpassword);
			}
			...
			if(isset($_REQUEST['candidate_portal_request']))
			{
				$rs = $authdb->Execute("SELECT *
									...
									AND mportalid = '".$_REQUEST['candidate_portal_request']."'");
			...
			else
			{
				...
				{		
					$rs = $authdb->Execute("SELECT *
                                     ...
                                     WHERE {$this->config->fielduser} = '".$this->ext_addslashes($extusername)."'
                                           AND {$this->config->fieldpass} = '".$this->ext_addslashes($extpassword)."'
										   AND mutype = '".$_REQUEST['usert_type_request']."'");
